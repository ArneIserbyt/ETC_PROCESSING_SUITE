---
title: "<h1 style='font-weight:900;'>Summary Ancillary Data for DE-HoH</h1>
...
DEHoH_LAI <- safe_read_add_columns(path, "BIFTAB__GRP_LAI__ICOSETC_DE-HoH_ANCILLARY__.*\\.csv")
DEHoH_LITTER_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_LITTER__ICOSETC_DE-HoH_ANCILLARY__.*\\.csv")
DEHoH_BIOMASS_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BIOMASS__ICOSETC_DE-HoH_ANCILLARY__.*\\.csv")
DEHoH_DBH <- safe_read_add_columns(path, "BIFTAB__GRP_DBH__ICOSETC_DE-HoH_ANCILLARY__.*\\.csv")
DEHoH_HEIGHT <- safe_read_add_columns(path, "BIFTAB__GRP_HEIGHTC__ICOSETC_DE-HoH_ANCILLARY__.*\\.csv")
DEHoH_SPP_O_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_SPP_O__ICOSETC_DE-HoH_ANCILLARY__.*\\.csv")
DEHoH_TREES_NUM_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_TREES_NUM__ICOSETC_DE-HoH_ANCILLARY__.*\\.csv")
DEHoH_BASAL_AREA_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BASAL_AREA__ICOSETC_DE-HoH_ANCILLARY__.*\\.csv")


### Select the most common species (>5% of species contribution)
SPP_Threshold <- DEHoH_SPP_O_L2data %>%
...
DEHoH_Spatial <- read_excel("I:/ETC_ancillary data/Labelled/Aggregated data/Aggregations_L2_Forests/DATABASE_exports/GRP_PLOT_AllSites.xlsx")
DEHoH_Spatial2 <- DEHoH_Spatial %>% 
  filter(Site == "DE-HoH")
...
transformed_data <- DEHoH_BIOMASS_L2data %>%
...
ggplot(Pop_TREE_data_reduced2, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Biomass_TonsPerHa - Biomass_TonsPerHa_SD, 
                ...
                color = "black") +
  labs(
    title = "Standing (dry) biomass at DE-HoH",
...
transformed_data2 <- DEHoH_BASAL_AREA_L2data %>%
...
ggplot(Pop_TREE_data_reduced2ba, aes(x = TREE_SPP, y = BasalArea_m2Ha, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = BasalArea_m2Ha - BasalArea_m2Ha_SD, 
                ...
                color = "black") +
  labs(
    title = "Basal Area (m²/ha) at DE-HoH ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data3 <- DEHoH_TREES_NUM_L2data %>%
...
ggplot(Pop_TREE_data_reduced3, aes(x = TREE_SPP, y = Density_TreesPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Density_TreesPerHa - Density_TreesPerHa_SD, 
                ...
                color = "black") +
  labs(
      title = "Number of trees per hectare at DE-HoH ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
    y = "Tree density (#/ha)",
    fill = "Plot_Year"
...
DEHoH_SPP_O_L2data_filtered <- DEHoH_SPP_O_L2data %>%
  filter(
...
DEHoH_SPP_O_L2data_sorted <- DEHoH_SPP_O_L2data_filtered %>%
  arrange(Plot_Year, desc(SPP_O))

DEHoH_SPP_O_L2data_sorted <- DEHoH_SPP_O_L2data_sorted %>%
  group_by(Plot_Year) %>%
  mutate(SPP_O_SPP = factor(SPP_O_SPP, levels = unique(SPP_O_SPP[order(SPP_O)])))  

unique_species <- unique(DEHoH_SPP_O_L2data_sorted$SPP_O_SPP)
...
ggplot(DEHoH_SPP_O_L2data_sorted, aes(x = Plot_Year, y = SPP_O, fill = SPP_O_SPP)) +
  ...
  labs(
    title = "Average tree species ratio per plot/campaign at DE-HoH",
    # x = "Plot Year",
...
ggplot(Pop_TREE_data_dominant, aes(x = TREE_DBH_CM, fill = Year, group = Year)) +
  ...
  labs(
    title = "DBH distribution per observation year within CPs at DE-HoH",
...
DEHoH_Spatial_filtered <- DEHoH_Spatial2 %>%
  select(PLOT_ID, PLOT_LOCATION_LAT, PLOT_LOCATION_LONG)
Biomass_PLOT <- Biomass_PLOT %>%
  left_join(DEHoH_Spatial_filtered, by = c("TREE_PLOT" = "PLOT_ID"))

#Double check: plot SP-I_03 turned into CP_05 during labelling. so it can be ignored here
Biomass_PLOT2 <- Biomass_PLOT %>%
 filter(TREE_PLOT != "SP-I_03")
...
ggplot() +
  ...
  labs(
    title = "Spatial variation in Biomass (tons/ha) at DE-HoH",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal() +
...
ggplot(Pop_TREE_data_reduced5, aes(x = TREE_SPP, y = DBH, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = DBH - DBH_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree DBH per species and plot/campaign at DE-HoH",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data4 <- DEHoH_HEIGHT %>%
...
ggplot(Pop_TREE_data_reduced4, aes(x = TREE_SPP, y = HEIGHTC, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = HEIGHTC - HEIGHTC_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree height per species and plot/campaign at DE-HoH",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
DEHoH_BIOMASS_DeadTrees <- DEHoH_BIOMASS_L2data %>% 
...
summarized_data <- DEHoH_BIOMASS_DeadTrees %>%
  group_by(TREE_SPP, Year, Plot, Plot_Year, BIOMASS_SPP, Species, BIOMASS_STATISTIC) %>%
  summarize(Biomass_TonsPerHa = mean(Biomass_TonsPerHa, na.rm = TRUE), .groups = 'drop')
...
ggplot(final_data, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  labs(
    title = "Biomass standing dead trees at DE-HoH",
    x = "", 
    y = "Biomass (tons/ha)",
    fill = "Plot_Year"
  ) +
...
filtered_data <- DEHoH_BIOMASS_L2data %>%
...
plot <- ggplot(filtered_data, aes(x = as.factor(Year), y = BIOMASS, fill = AGB_VEGTYPE)) +
  ...
  labs(
    ...
    title = "Annual variation in AGB understory at DE-HoH",
...
DEHoH_LAI$DATE2 <- ifelse(is.na(DEHoH_LAI$LAI_DATE), DEHoH_LAI$LAI_DATE_START, DEHoH_LAI$LAI_DATE)
...
DEHoH_LAI_mean <- DEHoH_LAI %>% filter(LAI_STATISTIC == "Mean")
DEHoH_LAI_sd <- DEHoH_LAI %>% filter(LAI_STATISTIC == "Standard Deviation")

combined_data <- DEHoH_LAI_mean %>%
  rename(Mean_LAI = LAI) %>%
  select(DATE2, Mean_LAI) %>%
  inner_join(DEHoH_LAI_sd %>% rename(SD_LAI = LAI) %>% select(DATE2, SD_LAI), by = "DATE2")
...
plot <- ggplot(combined_data, aes(x = DATE2)) +
  ...
  labs(x = "Date", y = "LAI ± SD (m²/m²)", title = "Seasonal variation in LAI at DE-HoH") +
  scale_x_date(
    date_labels = "%d-%m-%Y",
...
DEHoH_LITTER <- DEHoH_LITTER_L2data %>%
...
DEHoH_LITTER_filtered <- DEHoH_LITTER %>%
  filter(Year != "2018")


filtered_data <- DEHoH_LITTER_filtered %>% ####Double check: adjust if needed
...
combined_plot <- (plot_coarse + plot_fine_non) +
  plot_layout(ncol = 2) + 
  plot_annotation(
    title = "Mean (± SD) annual variation in dry litter mass at DE-HoH",  
    subtitle = "(n = number of CPs)",  
    theme = theme(
...
filtered_data2 <- DEHoH_LITTER_filtered %>% #### double check: delete _filtered
...
ggplot(filtered_data2, aes(x = Quarter, y = LITTER, fill = LITTER_ORGAN)) +
  geom_bar(stat = "identity", color = "black") +
  scale_fill_brewer(palette = "Accent") +
  labs(x = "Quarter", y = "Summed dry litter mass (kg/m²)", fill = "Litter Type", title = "Stacked non-woody litter per quarter at DE-HoH") +
  theme_minimal() +
  theme(
    legend.position = "bottom",
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, face = "bold"),
...
grid.text("\n Spatial variation in cumulative soil stock at DE-HoH",
...
DEHoH_VEG_CHEM_L2data <- read_latest_biftab("DE-HoH", data_dir)
DEHoH_VEG_CHEM_L2data <- DEHoH_VEG_CHEM_L2data %>%
  mutate(
    across(
...
plot_LMA <- function(df) {
  ...
  ggplot(combined_data, aes(x = DATE2, color = VEG_CHEM_SPP)) +
    ...
    labs(
      title = "Leaf Mass Area at DE-HoH",
      x = "Date",
      y = "LMA ± SD (kg/m²)",
      color = "Species"
    ) +
  ...
  plot_LMA(DEHoH_VEG_CHEM_L2data)
...
# Maak de plots
plot_C <- plot_conc(DEHoH_VEG_CHEM_L2data, "VEG_CHEM_CONC_C", "C concentration ± SD (gC/kg)")
plot_N <- plot_conc(DEHoH_VEG_CHEM_L2data, "VEG_CHEM_CONC_N", "N concentration ± SD (gN/kg)")


combined_plot <- (plot_C + plot_spacer() + plot_N) +
  plot_layout(ncol = 3, widths = c(1, 0.05, 1), guides = "collect") +
  plot_annotation(
    title = "Leaf nutrient analysis for C and N at DE-HoH",
    theme = theme(
      plot.title = element_text(face = "bold", size = 14, hjust = 0.5),
      legend.position = "none",
      legend.margin = margin(t = 30, b = 0, l = 0, r = 0)
...
library(dplyr)
 ...
 df <- read_phenocam("I:/Phenocam/DE-HoH/ROI/DE-HoH_DB_1001_3day.csv")
...
title(main = "Phenocam based phenology for DE-HoH", font.main = 2, line = 4)
...
text(
 ...
 phenology_dates <- phenophases(df, out_dir = "I:/Phenocam/DE-HoH/ROI/", internal=FALSE)
...
- Daily resolution = half hourly for 24h (48 images/day).
