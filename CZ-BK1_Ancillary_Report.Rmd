---
title: "<h1 style='font-weight:900;'>Summary Ancillary Data for CZ-BK1</h1>
...
CZBK1_LAI <- read.csv("I:/ETC_ancillary data/Labelled/Aggregated data/Aggregations_L2_Forests/MOST_RECENT_BIFTAB_FOREST/BIFTAB__GRP_LAI__ICOSETC_CZ-BK1_ANCILLARY__20260223113707.csv")
#CZBK1_LITTER_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_LITTER__ICOSETC_CZ-BK1_ANCILLARY__.*\\.csv")
CZBK1_BIOMASS_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BIOMASS__ICOSETC_CZ-BK1_ANCILLARY__.*\\.csv")
CZBK1_DBH <- safe_read_add_columns(path, "BIFTAB__GRP_DBH__ICOSETC_CZ-BK1_ANCILLARY__.*\\.csv")
CZBK1_HEIGHT <- safe_read_add_columns(path, "BIFTAB__GRP_HEIGHTC__ICOSETC_CZ-BK1_ANCILLARY__.*\\.csv")
CZBK1_SPP_O_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_SPP_O__ICOSETC_CZ-BK1_ANCILLARY__.*\\.csv")
CZBK1_TREES_NUM_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_TREES_NUM__ICOSETC_CZ-BK1_ANCILLARY__.*\\.csv")
CZBK1_BASAL_AREA_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BASAL_AREA__ICOSETC_CZ-BK1_ANCILLARY__.*\\.csv")


### Select the most common species (>5% of species contribution)
SPP_Threshold <- CZBK1_SPP_O_L2data %>%
...
TREEs_Site_CPs <- TREEs_Site %>%
  filter(
    grepl("^CP", TREE_PLOT) ) %>%
  mutate(Year = ifelse(Year == "2019", "2020", Year)) #unhashtag if needed for this site
...
CZBK1_Spatial <- read_excel("I:/ETC_ancillary data/Labelled/Aggregated data/Aggregations_L2_Forests/DATABASE_exports/GRP_PLOT_AllSites.xlsx")
CZBK1_Spatial2 <- CZBK1_Spatial %>% 
  filter(Site == "CZ-BK1")
...
transformed_data <- CZBK1_BIOMASS_L2data %>%
...
ggplot(Pop_TREE_data_reduced2, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Biomass_TonsPerHa - Biomass_TonsPerHa_SD, 
                ...
                color = "black") +
  labs(
    title = "Standing (dry) biomass at CZ-BK1",
...
transformed_data2 <- CZBK1_BASAL_AREA_L2data %>%
...
ggplot(Pop_TREE_data_reduced2ba, aes(x = TREE_SPP, y = BasalArea_m2Ha, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = BasalArea_m2Ha - BasalArea_m2Ha_SD, 
                ...
                color = "black") +
  labs(
    title = "Basal Area (m²/ha) at CZ-BK1 ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data3 <- CZBK1_TREES_NUM_L2data %>%
...
ggplot(Pop_TREE_data_reduced3, aes(x = TREE_SPP, y = Density_TreesPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Density_TreesPerHa - Density_TreesPerHa_SD, 
                ...
                color = "black") +
  labs(
      title = "Number of trees per hectare at CZ-BK1 ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
    y = "Tree density (#/ha)",
    fill = "Plot_Year"
...
CZBK1_SPP_O_L2data_filtered <- CZBK1_SPP_O_L2data %>%
  filter(
...
CZBK1_SPP_O_L2data_sorted <- CZBK1_SPP_O_L2data_filtered %>%
  arrange(Plot_Year, desc(SPP_O))

CZBK1_SPP_O_L2data_sorted <- CZBK1_SPP_O_L2data_sorted %>%
  group_by(Plot_Year) %>%
  mutate(SPP_O_SPP = factor(SPP_O_SPP, levels = unique(SPP_O_SPP[order(SPP_O)])))  

unique_species <- unique(CZBK1_SPP_O_L2data_sorted$SPP_O_SPP)
...
ggplot(CZBK1_SPP_O_L2data_sorted, aes(x = Plot_Year, y = SPP_O, fill = SPP_O_SPP)) +
  ...
  labs(
    title = "Average tree species ratio per plot/campaign at CZ-BK1",
    # x = "Plot Year",
...
ggplot(Pop_TREE_data_dominant, aes(x = TREE_DBH_CM, fill = Year, group = Year)) +
  ...
  labs(
    title = "DBH distribution per observation year within CPs at CZ-BK1",
...
CZBK1_Spatial_filtered <- CZBK1_Spatial2 %>%
  select(PLOT_ID, PLOT_LOCATION_LAT, PLOT_LOCATION_LONG)
Biomass_PLOT <- Biomass_PLOT %>%
  left_join(CZBK1_Spatial_filtered, by = c("TREE_PLOT" = "PLOT_ID"))
...
ggplot() +
  ...
  labs(
    title = "Spatial variation in Biomass (tons/ha) at CZ-BK1",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal() +
...
ggplot(Pop_TREE_data_reduced5, aes(x = TREE_SPP, y = DBH, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = DBH - DBH_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree DBH per species and plot/campaign at CZ-BK1",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data4 <- CZBK1_HEIGHT %>%
...
ggplot(Pop_TREE_data_reduced4, aes(x = TREE_SPP, y = HEIGHTC, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = HEIGHTC - HEIGHTC_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree height per species and plot/campaign at CZ-BK1",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
CZBK1_BIOMASS_DeadTrees <- CZBK1_BIOMASS_L2data %>% 
...
summarized_data <- CZBK1_BIOMASS_DeadTrees %>%
  group_by(TREE_SPP, Year, Plot, Plot_Year, BIOMASS_SPP, Species, BIOMASS_STATISTIC) %>%
  summarize(Biomass_TonsPerHa = mean(Biomass_TonsPerHa, na.rm = TRUE), .groups = 'drop')
...
ggplot(final_data, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  labs(
    title = "Biomass standing dead trees at CZ-BK1",
    x = "", 
    y = "Biomass (tons/ha)",
    fill = "Plot_Year"
  ) +
...
filtered_data <- CZBK1_BIOMASS_L2data %>%
...
plot <- ggplot(filtered_data, aes(x = as.factor(Year), y = BIOMASS, fill = AGB_VEGTYPE)) +
  ...
  labs(
    ...
    title = "Annual variation in AGB understory at CZ-BK1",
...
CZBK1_LAI$DATE2 <- ifelse(is.na(CZBK1_LAI$LAI_DATE), CZBK1_LAI$LAI_DATE_START, CZBK1_LAI$LAI_DATE)

#only data from CP - thus excluding SP-I DHPs from labelling
CZBK1_LAI <- CZBK1_LAI %>%
  filter(LAI_STATISTIC_NUMBER != "20")

CZBK1_LAI_mean <- CZBK1_LAI %>% filter(LAI_STATISTIC == "Mean")
CZBK1_LAI_sd <- CZBK1_LAI %>% filter(LAI_STATISTIC == "Standard Deviation")

combined_data <- CZBK1_LAI_mean %>%
  rename(Mean_LAI = LAI) %>%
  select(DATE2, Mean_LAI) %>%
  inner_join(CZBK1_LAI_sd %>% rename(SD_LAI = LAI) %>% select(DATE2, SD_LAI), by = "DATE2")
...
plot <- ggplot(combined_data, aes(x = DATE2)) +
  ...
  labs(x = "Date", y = "LAI ± SD (m²/m²)", title = "Seasonal variation in LAI at CZ-BK1") +
  scale_x_date(
    date_labels = "%d-%m-%Y",
...
<p style="font-size: 12px;">
CZ-BK1 is a Class2 site, hence litter is not collected.
...
grid.text("\n Spatial variation in cumulative soil stock at CZ-BK1",
...
CZBK1_VEG_CHEM_L2data <- read_latest_biftab("CZ-BK1", data_dir)
CZBK1_VEG_CHEM_L2data <- CZBK1_VEG_CHEM_L2data %>%
  mutate(
    across(
...
plot_LMA <- function(df) {
  ...
  ggplot(combined_data, aes(x = DATE2, color = VEG_CHEM_SPP)) +
    ...
    labs(
      title = "Leaf Mass Area at CZ-BK1",
      x = "Date",
      y = "LMA ± SD (kg/m²)",
      color = "Species"
    ) +
  ...
  plot_LMA(CZBK1_VEG_CHEM_L2data)
...
# Maak de plots
plot_C <- plot_conc(CZBK1_VEG_CHEM_L2data, "VEG_CHEM_CONC_C", "C concentration ± SD (gC/kg)")
plot_N <- plot_conc(CZBK1_VEG_CHEM_L2data, "VEG_CHEM_CONC_N", "N concentration ± SD (gN/kg)")


combined_plot <- (plot_C + plot_spacer() + plot_N) +
  plot_layout(ncol = 3, widths = c(1, 0.05, 1), guides = "collect") +
  plot_annotation(
    title = "Leaf nutrient analysis for C and N at CZ-BK1",
    theme = theme(
      plot.title = element_text(face = "bold", size = 14, hjust = 0.5),
      legend.position = "none",
      legend.margin = margin(t = 30, b = 0, l = 0, r = 0)
...
library(dplyr)
 ...
 df <- read_phenocam("I:/Phenocam/CZ-BK1/ROI/CZ-BK1_DB_1008_3day.csv")
...
title(main = "Phenocam based phenology at CZ-BK1", font.main = 2, line = 4)
...
- Several gaps in the time series. Large gap in winter 2021/2022.
