---
title: "<h1 style='font-weight:900;'>Summary Ancillary Data for FR-Hes</h1>
...
FRHes_LAI <- read.csv("I:/ETC_ancillary data/Labelled/Aggregated data/Aggregations_L2_Forests/MOST_RECENT_BIFTAB_FOREST/BIFTAB__GRP_LAI__ICOSETC_FR-Hes_ANCILLARY__20260224143838.csv")
FRHes_LITTER_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_LITTER__ICOSETC_FR-Hes_ANCILLARY__.*\\.csv")
FRHes_BIOMASS_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BIOMASS__ICOSETC_FR-Hes_ANCILLARY__.*\\.csv")
FRHes_DBH <- safe_read_add_columns(path, "BIFTAB__GRP_DBH__ICOSETC_FR-Hes_ANCILLARY__.*\\.csv")
FRHes_HEIGHT <- safe_read_add_columns(path, "BIFTAB__GRP_HEIGHTC__ICOSETC_FR-Hes_ANCILLARY__.*\\.csv")
FRHes_SPP_O_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_SPP_O__ICOSETC_FR-Hes_ANCILLARY__.*\\.csv")
FRHes_TREES_NUM_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_TREES_NUM__ICOSETC_FR-Hes_ANCILLARY__.*\\.csv")
FRHes_BASAL_AREA_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BASAL_AREA__ICOSETC_FR-Hes_ANCILLARY__.*\\.csv")



### Select the most common species (>5% of species contribution)
SPP_Threshold <- FRHes_SPP_O_L2data %>%
...
FRHes_Spatial <- read_excel("I:/ETC_ancillary data/Labelled/Aggregated data/Aggregations_L2_Forests/DATABASE_exports/GRP_PLOT_AllSites.xlsx")
FRHes_Spatial2 <- FRHes_Spatial %>% 
  filter(Site == "FR-Hes")
...
transformed_data <- FRHes_BIOMASS_L2data %>%
...
ggplot(Pop_TREE_data_reduced2, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Biomass_TonsPerHa - Biomass_TonsPerHa_SD, 
                ...
                color = "black") +
  labs(
    title = "Standing (dry) biomass at FR-Hes",
...
transformed_data2 <- FRHes_BASAL_AREA_L2data %>%
...
ggplot(Pop_TREE_data_reduced2ba, aes(x = TREE_SPP, y = BasalArea_m2Ha, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = BasalArea_m2Ha - BasalArea_m2Ha_SD, 
                ...
                color = "black") +
  labs(
    title = "Basal Area (m²/ha) at FR-Hes ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data3 <- FRHes_TREES_NUM_L2data %>%
...
ggplot(Pop_TREE_data_reduced3, aes(x = TREE_SPP, y = Density_TreesPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Density_TreesPerHa - Density_TreesPerHa_SD, 
                ...
                color = "black") +
  labs(
      title = "Number of trees per hectare at FR-Hes ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
    y = "Tree density (#/ha)",
    fill = "Plot_Year"
...
FRHes_SPP_O_L2data_filtered <- FRHes_SPP_O_L2data %>%
  filter(
...
FRHes_SPP_O_L2data_sorted <- FRHes_SPP_O_L2data_filtered %>%
  arrange(Plot_Year, desc(SPP_O))

FRHes_SPP_O_L2data_sorted <- FRHes_SPP_O_L2data_sorted %>%
  group_by(Plot_Year) %>%
  mutate(SPP_O_SPP = factor(SPP_O_SPP, levels = unique(SPP_O_SPP[order(SPP_O)])))  

unique_species <- unique(FRHes_SPP_O_L2data_sorted$SPP_O_SPP)
...
ggplot(FRHes_SPP_O_L2data_sorted, aes(x = Plot_Year, y = SPP_O, fill = SPP_O_SPP)) +
  ...
  labs(
    title = "Average tree species ratio per plot/campaign at FR-Hes",
    # x = "Plot Year",
...
ggplot(Pop_TREE_data_dominant, aes(x = TREE_DBH_CM, fill = Year, group = Year)) +
  ...
  labs(
    title = "DBH distribution per observation year within CPs at FR-Hes",
...
FRHes_Spatial_filtered <- FRHes_Spatial2 %>%
  select(PLOT_ID, PLOT_LOCATION_LAT, PLOT_LOCATION_LONG)
Biomass_PLOT <- Biomass_PLOT %>%
  left_join(FRHes_Spatial_filtered, by = c("TREE_PLOT" = "PLOT_ID"))
...
ggplot() +
  ...
  labs(
    title = "Spatial variation in Biomass (tons/ha) at FR-Hes",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal() +
...
ggplot(Pop_TREE_data_reduced5, aes(x = TREE_SPP, y = DBH, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = DBH - DBH_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree DBH per species and plot/campaign at FR-Hes",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data4 <- FRHes_HEIGHT %>%
...
ggplot(Pop_TREE_data_reduced4, aes(x = TREE_SPP, y = HEIGHTC, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = HEIGHTC - HEIGHTC_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree height per species and plot/campaign at FR-Hes",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
FRHes_BIOMASS_DeadTrees <- FRHes_BIOMASS_L2data %>% 
...
summarized_data <- FRHes_BIOMASS_DeadTrees %>%
  group_by(TREE_SPP, Year, Plot, Plot_Year, BIOMASS_SPP, Species, BIOMASS_STATISTIC) %>%
  summarize(Biomass_TonsPerHa = mean(Biomass_TonsPerHa, na.rm = TRUE), .groups = 'drop')
...
ggplot(final_data, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  labs(
    title = "Biomass standing dead trees at FR-Hes",
    x = "", 
    y = "Biomass (tons/ha)",
    fill = "Plot_Year"
  ) +
...
filtered_data <- FRHes_BIOMASS_L2data %>%
...
plot <- ggplot(filtered_data, aes(x = as.factor(Year), y = BIOMASS, fill = AGB_VEGTYPE)) +
  ...
  labs(
    ...
    title = "Annual variation in AGB understory at FR-Hes",
...
FRHes_LAI$DATE2 <- ifelse(is.na(FRHes_LAI$LAI_DATE), FRHes_LAI$LAI_DATE_START, FRHes_LAI$LAI_DATE)
...
FRHes_LAI_mean <- FRHes_LAI %>% filter(LAI_STATISTIC == "Mean")
FRHes_LAI_sd <- FRHes_LAI %>% filter(LAI_STATISTIC == "Standard Deviation")

combined_data <- FRHes_LAI_mean %>%
  rename(Mean_LAI = LAI) %>%
  select(DATE2, Mean_LAI) %>%
  inner_join(FRHes_LAI_sd %>% rename(SD_LAI = LAI) %>% select(DATE2, SD_LAI), by = "DATE2")
...
plot <- ggplot(combined_data, aes(x = DATE2)) +
  ...
  labs(x = "Date", y = "LAI ± SD (m²/m²)", title = "Seasonal variation in LAI at FR-Hes") +
  scale_x_date(
    date_labels = "%d-%m-%Y",
...
FRHes_LITTER <- FRHes_LITTER_L2data %>%
...
FRHes_LITTER_filtered <- FRHes_LITTER %>%
  filter(Year != "2021")



filtered_data <- FRHes_LITTER_filtered %>% ####Double check: adjust if needed
...
combined_plot <- (plot_coarse + plot_fine_non) +
  plot_layout(ncol = 2) + 
  plot_annotation(
    title = "Mean (± SD) annual variation in dry litter mass at FR-Hes",  
    subtitle = "(n = number of CPs)",  
    theme = theme(
...
filtered_data2 <- FRHes_LITTER %>% #### double check: delete _filtered
...
ggplot(filtered_data2, aes(x = Quarter, y = LITTER, fill = LITTER_ORGAN)) +
  geom_bar(stat = "identity", color = "black") +
  scale_fill_brewer(palette = "Accent") +
  labs(x = "Quarter", y = "Summed dry litter mass (kg/m²)", fill = "Litter Type", title = "Stacked non-woody litter per quarter at FR-Hes") +
  theme_minimal() +
  theme(
    legend.position = "bottom",
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, face = "bold"),
...
Soil data collection is currently in progress for FR-Hes.
...
FRHes_VEG_CHEM_L2data <- read_latest_biftab("FR-Hes", data_dir)
FRHes_VEG_CHEM_L2data <- FRHes_VEG_CHEM_L2data %>%
  mutate(
    across(
...
plot_LMA <- function(df) {
  ...
  ggplot(combined_data, aes(x = DATE2, color = VEG_CHEM_SPP)) +
    ...
    labs(
      title = "Leaf Mass Area at FR-Hes",
      x = "Date",
      y = "LMA ± SD (kg/m²)",
      color = "Species"
    ) +
  ...
  plot_LMA(FRHes_VEG_CHEM_L2data)
...
# Maak de plots
plot_C <- plot_conc(FRHes_VEG_CHEM_L2data, "VEG_CHEM_CONC_C", "C concentration ± SD (gC/kg)")
plot_N <- plot_conc(FRHes_VEG_CHEM_L2data, "VEG_CHEM_CONC_N", "N concentration ± SD (gN/kg)")


combined_plot <- (plot_C + plot_spacer() + plot_N) +
  plot_layout(ncol = 3, widths = c(1, 0.05, 1), guides = "collect") +
  plot_annotation(
    title = "Leaf nutrient analysis for C and N at FR-Hes",
    theme = theme(
      plot.title = element_text(face = "bold", size = 14, hjust = 0.5),
...
<p style="font-size: 12px;">
Time series from 2011-2022 available for FR-Hes, but issue with the image format.
