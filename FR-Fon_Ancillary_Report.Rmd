---
title: "<h1 style='font-weight:900;'>Summary Ancillary Data for FR-Fon</h1>
...
FRFon_LAI <- safe_read_add_columns(path, "BIFTAB__GRP_LAI__ICOSETC_FR-Fon_ANCILLARY__.*\\.csv")
FRFon_LITTER_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_LITTER__ICOSETC_FR-Fon_ANCILLARY__.*\\.csv")
FRFon_BIOMASS_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BIOMASS__ICOSETC_FR-Fon_ANCILLARY__.*\\.csv")
FRFon_DBH <- safe_read_add_columns(path, "BIFTAB__GRP_DBH__ICOSETC_FR-Fon_ANCILLARY__.*\\.csv")
FRFon_HEIGHT <- safe_read_add_columns(path, "BIFTAB__GRP_HEIGHTC__ICOSETC_FR-Fon_ANCILLARY__.*\\.csv")
FRFon_SPP_O_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_SPP_O__ICOSETC_FR-Fon_ANCILLARY__.*\\.csv")
FRFon_TREES_NUM_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_TREES_NUM__ICOSETC_FR-Fon_ANCILLARY__.*\\.csv")
FRFon_BASAL_AREA_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BASAL_AREA__ICOSETC_FR-Fon_ANCILLARY__.*\\.csv")


### Select the most common species (>5% of species contribution)
SPP_Threshold <- FRFon_SPP_O_L2data %>%
...
TREEs_Site_CPs <- TREEs_Site %>%
  filter(
    grepl("^CP", TREE_PLOT) )
# %>%
#   mutate(Year = ifelse(Year == "2017", "2020", Year)) #unhashtag if needed for this site
...
FRFon_Spatial <- read_excel("I:/ETC_ancillary data/Labelled/Aggregated data/Aggregations_L2_Forests/DATABASE_exports/GRP_PLOT_AllSites.xlsx")
FRFon_Spatial2 <- FRFon_Spatial %>% 
  filter(Site == "FR-Fon")
...
transformed_data <- FRFon_BIOMASS_L2data %>%
...
ggplot(Pop_TREE_data_reduced2, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Biomass_TonsPerHa - Biomass_TonsPerHa_SD, 
                ...
                color = "black") +
  labs(
    title = "Standing (dry) biomass at FR-Fon",
...
transformed_data2 <- FRFon_BASAL_AREA_L2data %>%
...
ggplot(Pop_TREE_data_reduced2ba, aes(x = TREE_SPP, y = BasalArea_m2Ha, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = BasalArea_m2Ha - BasalArea_m2Ha_SD, 
                ...
                color = "black") +
  labs(
    title = "Basal Area (m²/ha) at FR-Fon ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data3 <- FRFon_TREES_NUM_L2data %>%
...
ggplot(Pop_TREE_data_reduced3, aes(x = TREE_SPP, y = Density_TreesPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Density_TreesPerHa - Density_TreesPerHa_SD, 
                ...
                color = "black") +
  labs(
      title = "Number of trees per hectare at FR-Fon ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
    y = "Tree density (#/ha)",
    fill = "Plot_Year"
...
FRFon_SPP_O_L2data_filtered <- FRFon_SPP_O_L2data %>%
  filter(
...
FRFon_SPP_O_L2data_sorted <- FRFon_SPP_O_L2data_filtered %>%
  arrange(Plot_Year, desc(SPP_O))

FRFon_SPP_O_L2data_sorted <- FRFon_SPP_O_L2data_sorted %>%
  group_by(Plot_Year) %>%
  mutate(SPP_O_SPP = factor(SPP_O_SPP, levels = unique(SPP_O_SPP[order(SPP_O)])))  

unique_species <- unique(FRFon_SPP_O_L2data_sorted$SPP_O_SPP)
...
ggplot(FRFon_SPP_O_L2data_sorted, aes(x = Plot_Year, y = SPP_O, fill = SPP_O_SPP)) +
  ...
  labs(
    title = "Average tree species ratio per plot/campaign at FR-Fon",
    # x = "Plot Year",
...
# Data voor de meest voorkomende soort
...
plot_dominant <- ggplot(Pop_TREE_data_dominant, aes(x = TREE_DBH_CM, fill = Year)) +
  geom_histogram(binwidth = 5, color = "black", position = "dodge", boundary = 0, alpha = 0.7, show.legend = FALSE) +  
  geom_density(aes(y = ..count..*5, color = Year), size = 0.5, alpha = 0, adjust = 1.5, position = "identity", show.legend = FALSE) +  
  labs(
    subtitle = bquote(~ italic(.(dominant_species))),
...
# Data voor Quercus petraea (Matt.) Liebl.
Quercus_data <- TREEs_Site_CPs %>%
  filter(TREE_SPP == "Quercus petraea (Matt.) Liebl.")

Quercus_data <- Quercus_data %>% mutate(Year = as.factor(Year))

plot_Quercus <- ggplot(Quercus_data, aes(x = TREE_DBH_CM, fill = Year)) +
  geom_histogram(binwidth = 10, color = "black", position = "dodge", boundary = 5, alpha = 0.7) +  
  geom_density(aes(y = ..count..*10, color = Year), size = 0.5, alpha = 0, adjust = 1.5, position = "identity") +  
  labs(
    subtitle = "Quercus petraea (Matt.) Liebl.",
    x = "DBH (cm)", y = "Tree count"
  ) +
  scale_x_continuous(breaks = seq(5, 115, by = 10), limits = c(5, 115)) +
  scale_fill_brewer(palette = "Accent") +  
  scale_color_brewer(palette = "Accent") +  
  theme_minimal() +
  theme(
...
# Combineer de plots
combined_plot <- (plot_dominant + plot_Quercus) +
  plot_layout(ncol = 2) + 
  plot_annotation(
    title = "DBH distribution per observation year within CPs at FR-Fon",
        theme = theme(
...
# Bekijk de gecombineerde grafiek
print(combined_plot)
...
FRFon_Spatial_filtered <- FRFon_Spatial2 %>%
  select(PLOT_ID, PLOT_LOCATION_LAT, PLOT_LOCATION_LONG)
Biomass_PLOT <- Biomass_PLOT %>%
  left_join(FRFon_Spatial_filtered, by = c("TREE_PLOT" = "PLOT_ID"))
...
ggplot() +
  ...
  labs(
    title = "Spatial variation in Biomass (tons/ha) at FR-Fon",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal() +
...
ggplot(Pop_TREE_data_reduced5, aes(x = TREE_SPP, y = DBH, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = DBH - DBH_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree DBH per species and plot/campaign at FR-Fon",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data4 <- FRFon_HEIGHT %>%
...
ggplot(Pop_TREE_data_reduced4, aes(x = TREE_SPP, y = HEIGHTC, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = HEIGHTC - HEIGHTC_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree height per species and plot/campaign at FR-Fon",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
FRFon_BIOMASS_DeadTrees <- FRFon_BIOMASS_L2data %>% 
...
summarized_data <- FRFon_BIOMASS_DeadTrees %>%
  group_by(TREE_SPP, Year, Plot, Plot_Year, BIOMASS_SPP, Species, BIOMASS_STATISTIC) %>%
  summarize(Biomass_TonsPerHa = mean(Biomass_TonsPerHa, na.rm = TRUE), .groups = 'drop')
...
ggplot(final_data, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  labs(
    title = "Biomass standing dead trees at FR-Fon",
    x = "", 
    y = "Biomass (tons/ha)",
    fill = "Plot_Year"
  ) +
...
filtered_data <- FRFon_BIOMASS_L2data %>%
...
plot <- ggplot(filtered_data, aes(x = as.factor(Year), y = BIOMASS, fill = AGB_VEGTYPE)) +
  ...
  labs(
    ...
    title = "Annual variation in AGB understory at FR-Fon",
...
FRFon_LAI$DATE2 <- ifelse(is.na(FRFon_LAI$LAI_DATE), FRFon_LAI$LAI_DATE_START, FRFon_LAI$LAI_DATE)
...
FRFon_LAI_mean <- FRFon_LAI %>% filter(LAI_STATISTIC == "Mean")
FRFon_LAI_sd <- FRFon_LAI %>% filter(LAI_STATISTIC == "Standard Deviation")

combined_data <- FRFon_LAI_mean %>%
  rename(Mean_LAI = LAI) %>%
  select(DATE2, Mean_LAI) %>%
  inner_join(FRFon_LAI_sd %>% rename(SD_LAI = LAI) %>% select(DATE2, SD_LAI), by = "DATE2")
...
plot <- ggplot(combined_data, aes(x = DATE2)) +
  ...
  labs(x = "Date", y = "LAI ± SD (m²/m²)", title = "Seasonal variation in LAI at FR-Fon") +
  scale_x_date(
    date_labels = "%d-%m-%Y",
...
FRFon_LITTER <- FRFon_LITTER_L2data %>%
...
FRFon_LITTER_filtered <- FRFon_LITTER %>%
   filter(Year != "2017")



filtered_data <- FRFon_LITTER_filtered %>% ####Double check: adjust if needed
...
combined_plot <- (plot_coarse + plot_fine_non) +
  plot_layout(ncol = 2) + 
  plot_annotation(
    title = "Mean (± SD) annual variation in dry litter mass at FR-Fon",  
    subtitle = "(n = number of CPs)",  
    theme = theme(
      plot.title = element_text(face = "bold", hjust = 0.5, size = 14),  
      plot.subtitle = element_text(hjust = 0.5, size = 10, margin = margin(b = 25))   
...
filtered_data2 <- FRFon_LITTER %>% #### double check: delete _filtered
...
ggplot(filtered_data2, aes(x = Quarter, y = LITTER, fill = LITTER_ORGAN)) +
  geom_bar(stat = "identity", color = "black") +
  scale_fill_brewer(palette = "Accent") +
  labs(x = "Quarter", y = "Summed dry litter mass (kg/m²)", fill = "Litter Type", title = "Stacked non-woody litter per quarter at FR-Fon") +
  theme_minimal() +
  theme(
    legend.position = "bottom",
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, face = "bold"),
...
grid.text("\n Spatial variation in cumulative soil stock at FR-Fon",
...
FRFon_VEG_CHEM_L2data <- read_latest_biftab("FR-Fon", data_dir)
FRFon_VEG_CHEM_L2data <- FRFon_VEG_CHEM_L2data %>%
  mutate(
    across(
...
plot_LMA <- function(df) {
  ...
  ggplot(combined_data, aes(x = DATE2, color = VEG_CHEM_SPP)) +
    ...
    labs(
      title = "Leaf Mass Area at FR-Fon",
      x = "Date",
      y = "LMA ± SD (kg/m²)",
      color = "Species"
    ) +
  ...
  plot_LMA(FRFon_VEG_CHEM_L2data)
...
# Maak de plots
plot_C <- plot_conc(FRFon_VEG_CHEM_L2data, "VEG_CHEM_CONC_C", "C concentration ± SD (gC/kg)")
plot_N <- plot_conc(FRFon_VEG_CHEM_L2data, "VEG_CHEM_CONC_N", "N concentration ± SD (gN/kg)")


combined_plot <- (plot_C + plot_spacer() + plot_N) +
  plot_layout(ncol = 3, widths = c(1, 0.05, 1), guides = "collect") +
  plot_annotation(
    title = "Leaf nutrient analysis for C and N at FR-Fon",
    theme = theme(
      plot.title = element_text(face = "bold", size = 14, hjust = 0.5),
      legend.position = "none",
      legend.margin = margin(t = 30, b = 0, l = 0, r = 0)
...
library(dplyr)
 ...
 df <- read_phenocam("I:/Phenocam/FR-Fon/ROI/FR-Fon_DB_1000_3day.csv")
...
title(main = "Phenocam based phenology for FR-Fon", font.main = 2, line = 4)
