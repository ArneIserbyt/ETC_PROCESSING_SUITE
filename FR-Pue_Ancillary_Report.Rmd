---
title: "<h1 style='font-weight:900;'>Summary Ancillary Data for FR-Pue</h1>
...
FRPue_LAI <- safe_read_add_columns(path, "BIFTAB__GRP_LAI__ICOSETC_FR-Pue_ANCILLARY__.*\\.csv")
FRPue_LITTER_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_LITTER__ICOSETC_FR-Pue_ANCILLARY__.*\\.csv")
FRPue_BIOMASS_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BIOMASS__ICOSETC_FR-Pue_ANCILLARY__.*\\.csv")
FRPue_DBH <- safe_read_add_columns(path, "BIFTAB__GRP_DBH__ICOSETC_FR-Pue_ANCILLARY__.*\\.csv")
FRPue_HEIGHT <- safe_read_add_columns(path, "BIFTAB__GRP_HEIGHTC__ICOSETC_FR-Pue_ANCILLARY__.*\\.csv")
FRPue_SPP_O_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_SPP_O__ICOSETC_FR-Pue_ANCILLARY__.*\\.csv")
FRPue_TREES_NUM_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_TREES_NUM__ICOSETC_FR-Pue_ANCILLARY__.*\\.csv")
FRPue_BASAL_AREA_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BASAL_AREA__ICOSETC_FR-Pue_ANCILLARY__.*\\.csv")


### Select the most common species (>5% of species contribution)
SPP_Threshold <- FRPue_SPP_O_L2data %>%
...
FRPue_Spatial <- read_excel("I:/ETC_ancillary data/Labelled/Aggregated data/Aggregations_L2_Forests/DATABASE_exports/GRP_PLOT_AllSites.xlsx")
FRPue_Spatial2 <- FRPue_Spatial %>% 
  filter(Site == "FR-Pue")
...
transformed_data <- FRPue_BIOMASS_L2data %>%
...
ggplot(Pop_TREE_data_reduced2, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Biomass_TonsPerHa - Biomass_TonsPerHa_SD, 
                ...
                color = "black") +
  labs(
    title = "Standing (dry) biomass at FR-Pue",
...
transformed_data2 <- FRPue_BASAL_AREA_L2data %>%
...
ggplot(Pop_TREE_data_reduced2ba, aes(x = TREE_SPP, y = BasalArea_m2Ha, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = BasalArea_m2Ha - BasalArea_m2Ha_SD, 
                ...
                color = "black") +
  labs(
    title = "Basal Area (m²/ha) at FR-Pue ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data3 <- FRPue_TREES_NUM_L2data %>%
...
ggplot(Pop_TREE_data_reduced3, aes(x = TREE_SPP, y = Density_TreesPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Density_TreesPerHa - Density_TreesPerHa_SD, 
                ...
                color = "black") +
  labs(
      title = "Number of trees per hectare at FR-Pue ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
    y = "Tree density (#/ha)",
    fill = "Plot_Year"
...
FRPue_SPP_O_L2data_filtered <- FRPue_SPP_O_L2data %>%
  filter(
...
FRPue_SPP_O_L2data_sorted <- FRPue_SPP_O_L2data_filtered %>%
  arrange(Plot_Year, desc(SPP_O))

FRPue_SPP_O_L2data_sorted <- FRPue_SPP_O_L2data_sorted %>%
  group_by(Plot_Year) %>%
  mutate(SPP_O_SPP = factor(SPP_O_SPP, levels = unique(SPP_O_SPP[order(SPP_O)])))  

unique_species <- unique(FRPue_SPP_O_L2data_sorted$SPP_O_SPP)
...
ggplot(FRPue_SPP_O_L2data_sorted, aes(x = Plot_Year, y = SPP_O, fill = SPP_O_SPP)) +
  ...
  labs(
    title = "Average tree species ratio per plot/campaign at FR-Pue",
    # x = "Plot Year",
...
ggplot(Pop_TREE_data_dominant, aes(x = TREE_DBH_CM, fill = Year, group = Year)) +
  ...
  labs(
    title = "DBH distribution per observation year within CPs at FR-Pue",
...
Biomass_PLOT <- TREEs_Site2 %>%
  ...
  mutate(
    Area_ha = ifelse(startsWith(TREE_PLOT, "CP"), 0.2, 0.019635), #about 50% of the trees have 5 < DBH < 10. hence (78.5m²+314.2m²)/2 = 196.6m² is used for SPs 
    Biomass_TonsPerHa = round(Sum_Biomass_Ton / Area_ha, 2),
    Plot_Year = paste(TREE_PLOT, Year, sep = "_")
  )

FRPue_Spatial_filtered <- FRPue_Spatial2 %>%
  select(PLOT_ID, PLOT_LOCATION_LAT, PLOT_LOCATION_LONG)
Biomass_PLOT <- Biomass_PLOT %>%
  left_join(FRPue_Spatial_filtered, by = c("TREE_PLOT" = "PLOT_ID"))
...
ggplot() +
  ...
  labs(
    title = "Spatial variation in Biomass (tons/ha) at FR-Pue",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal() +
...
ggplot(Pop_TREE_data_reduced5, aes(x = TREE_SPP, y = DBH, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = DBH - DBH_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree DBH per species and plot/campaign at FR-Pue",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data4 <- FRPue_HEIGHT %>%
...
ggplot(Pop_TREE_data_reduced4, aes(x = TREE_SPP, y = HEIGHTC, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = HEIGHTC - HEIGHTC_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree height per species and plot/campaign at FR-Pue",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
FRPue_BIOMASS_DeadTrees <- FRPue_BIOMASS_L2data %>% 
...
summarized_data <- FRPue_BIOMASS_DeadTrees %>%
  group_by(TREE_SPP, Year, Plot, Plot_Year, BIOMASS_SPP, Species, BIOMASS_STATISTIC) %>%
  summarize(Biomass_TonsPerHa = mean(Biomass_TonsPerHa, na.rm = TRUE), .groups = 'drop')
...
ggplot(final_data, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  labs(
    title = "Biomass standing dead trees at FR-Pue",
    x = "", 
    y = "Biomass (tons/ha)",
    fill = "Plot_Year"
  ) +
...
filtered_data <- FRPue_BIOMASS_L2data %>%
...
plot <- ggplot(filtered_data, aes(x = as.factor(Year), y = BIOMASS, fill = AGB_VEGTYPE)) +
  ...
  labs(
    ...
    title = "Annual variation in AGB understory at FR-Pue",
...
FRPue_LAI$DATE2 <- ifelse(is.na(FRPue_LAI$LAI_DATE), FRPue_LAI$LAI_DATE_START, FRPue_LAI$LAI_DATE)
...
FRPue_LAI_mean <- FRPue_LAI %>% filter(LAI_STATISTIC == "Mean")
FRPue_LAI_sd <- FRPue_LAI %>% filter(LAI_STATISTIC == "Standard Deviation")

combined_data <- FRPue_LAI_mean %>%
  rename(Mean_LAI = LAI) %>%
  select(DATE2, Mean_LAI) %>%
  inner_join(FRPue_LAI_sd %>% rename(SD_LAI = LAI) %>% select(DATE2, SD_LAI), by = "DATE2")
...
plot <- ggplot(combined_data, aes(x = DATE2)) +
  ...
  labs(x = "Date", y = "LAI ± SD (m²/m²)", title = "Seasonal variation in LAI at FR-Pue") +
  scale_x_date(
    date_labels = "%d-%m-%Y",
...
<p style="font-size: 12px;">
FR-Pue is a Class2 site, hence litter is not collected.
...
Soil data collection is currently in progress for FR-Pue.
...
FRPue_VEG_CHEM_L2data <- read_latest_biftab("FR-Pue", data_dir)
FRPue_VEG_CHEM_L2data <- FRPue_VEG_CHEM_L2data %>%
  mutate(
    across(
...
plot_LMA <- function(df) {
  ...
  ggplot(combined_data, aes(x = DATE2, color = VEG_CHEM_SPP)) +
    ...
    labs(
      title = "Leaf Mass Area at FR-Pue",
      x = "Date",
      y = "LMA ± SD (kg/m²)",
      color = "Species"
    ) +
  ...
  plot_LMA(FRPue_VEG_CHEM_L2data)
...
# Maak de plots
plot_C <- plot_conc(FRPue_VEG_CHEM_L2data, "VEG_CHEM_CONC_C", "C concentration ± SD (gC/kg)")
plot_N <- plot_conc(FRPue_VEG_CHEM_L2data, "VEG_CHEM_CONC_N", "N concentration ± SD (gN/kg)")


combined_plot <- (plot_C + plot_spacer() + plot_N) +
  plot_layout(ncol = 3, widths = c(1, 0.05, 1), guides = "collect") +
  plot_annotation(
    title = "Leaf nutrient analysis for C and N at FR-Pue",
    theme = theme(
      plot.title = element_text(face = "bold", size = 14, hjust = 0.5),
...
<p style="font-size: 12px;">
ETC did not yet receive phenocam images for FR-Pue.
