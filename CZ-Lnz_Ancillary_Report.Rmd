---
title: "<h1 style='font-weight:900;'>Summary Ancillary Data for CZ-Lnz</h1>
...
CZLnz_LAI <- safe_read_add_columns(path, "BIFTAB__GRP_LAI__ICOSETC_CZ-Lnz_ANCILLARY__.*\\.csv")
CZLnz_LITTER_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_LITTER__ICOSETC_CZ-Lnz_ANCILLARY__.*\\.csv")
CZLnz_BIOMASS_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BIOMASS__ICOSETC_CZ-Lnz_ANCILLARY__.*\\.csv")
CZLnz_DBH <- safe_read_add_columns(path, "BIFTAB__GRP_DBH__ICOSETC_CZ-Lnz_ANCILLARY__.*\\.csv")
CZLnz_HEIGHT <- safe_read_add_columns(path, "BIFTAB__GRP_HEIGHTC__ICOSETC_CZ-Lnz_ANCILLARY__.*\\.csv")
CZLnz_SPP_O_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_SPP_O__ICOSETC_CZ-Lnz_ANCILLARY__.*\\.csv")
CZLnz_TREES_NUM_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_TREES_NUM__ICOSETC_CZ-Lnz_ANCILLARY__.*\\.csv")
CZLnz_BASAL_AREA_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BASAL_AREA__ICOSETC_CZ-Lnz_ANCILLARY__.*\\.csv")


### Select the most common species (>5% of species contribution)
SPP_Threshold <- CZLnz_SPP_O_L2data %>%
...
TREEs_Site_CPs <- TREEs_Site %>%
  filter(
    grepl("^CP", TREE_PLOT) ) %>%
  mutate(Year = ifelse(Year == "2022", "2020", Year)) #unhashtag if needed for this site
...
CZLnz_Spatial <- read_excel("I:/ETC_ancillary data/Labelled/Aggregated data/Aggregations_L2_Forests/DATABASE_exports/GRP_PLOT_AllSites.xlsx")
CZLnz_Spatial2 <- CZLnz_Spatial %>% 
  filter(Site == "CZ-Lnz")
...
transformed_data <- CZLnz_BIOMASS_L2data %>%
...
ggplot(Pop_TREE_data_reduced2, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Biomass_TonsPerHa - Biomass_TonsPerHa_SD, 
                ...
                color = "black") +
  labs(
    title = "Standing (dry) biomass at CZ-Lnz",
...
transformed_data2 <- CZLnz_BASAL_AREA_L2data %>%
...
ggplot(Pop_TREE_data_reduced2ba, aes(x = TREE_SPP, y = BasalArea_m2Ha, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = BasalArea_m2Ha - BasalArea_m2Ha_SD, 
                ...
                color = "black") +
  labs(
    title = "Basal Area (m²/ha) at CZ-Lnz ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data3 <- CZLnz_TREES_NUM_L2data %>%
...
ggplot(Pop_TREE_data_reduced3, aes(x = TREE_SPP, y = Density_TreesPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Density_TreesPerHa - Density_TreesPerHa_SD, 
                ...
                color = "black") +
  labs(
      title = "Number of trees per hectare at CZ-Lnz ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
    y = "Tree density (#/ha)",
    fill = "Plot_Year"
...
CZLnz_SPP_O_L2data_filtered <- CZLnz_SPP_O_L2data %>%
  filter(
...
CZLnz_SPP_O_L2data_sorted <- CZLnz_SPP_O_L2data_filtered %>%
  arrange(Plot_Year, desc(SPP_O))

CZLnz_SPP_O_L2data_sorted <- CZLnz_SPP_O_L2data_sorted %>%
  group_by(Plot_Year) %>%
  mutate(SPP_O_SPP = factor(SPP_O_SPP, levels = unique(SPP_O_SPP[order(SPP_O)])))  

unique_species <- unique(CZLnz_SPP_O_L2data_sorted$SPP_O_SPP)
...
ggplot(CZLnz_SPP_O_L2data_sorted, aes(x = Plot_Year, y = SPP_O, fill = SPP_O_SPP)) +
  ...
  labs(
    title = "Average tree species ratio per plot/campaign at CZ-Lnz",
    # x = "Plot Year",
...
ggplot(Pop_TREE_data_dominant, aes(x = TREE_DBH_CM, fill = Year, group = Year)) +
  ...
  labs(
    title = "DBH distribution per observation year within CPs at CZ-Lnz",
...
CZLnz_Spatial_filtered <- CZLnz_Spatial2 %>%
  select(PLOT_ID, PLOT_LOCATION_LAT, PLOT_LOCATION_LONG)
Biomass_PLOT <- Biomass_PLOT %>%
  left_join(CZLnz_Spatial_filtered, by = c("TREE_PLOT" = "PLOT_ID"))
...
Biomass_PLOT_labelling <- Biomass_PLOT2 %>%
  filter(Year == "2020")
...
ggplot() +
  ...
  labs(
    title = "Spatial variation in Biomass (tons/ha) at CZ-Lnz",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal() +
...
ggplot(Pop_TREE_data_reduced5, aes(x = TREE_SPP, y = DBH, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = DBH - DBH_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree DBH per species and plot/campaign at CZ-Lnz",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data4 <- CZLnz_HEIGHT %>%
...
ggplot(Pop_TREE_data_reduced4, aes(x = TREE_SPP, y = HEIGHTC, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = HEIGHTC - HEIGHTC_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree height per species and plot/campaign at CZ-Lnz",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
CZLnz_BIOMASS_DeadTrees <- CZLnz_BIOMASS_L2data %>% 
...
summarized_data <- CZLnz_BIOMASS_DeadTrees %>%
  group_by(TREE_SPP, Year, Plot, Plot_Year, BIOMASS_SPP, Species, BIOMASS_STATISTIC) %>%
  summarize(Biomass_TonsPerHa = mean(Biomass_TonsPerHa, na.rm = TRUE), .groups = 'drop')
...
ggplot(final_data, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  labs(
    title = "Biomass standing dead trees at CZ-Lnz",
    x = "", 
    y = "Biomass (tons/ha)",
    fill = "Plot_Year"
  ) +
...
filtered_data <- CZLnz_BIOMASS_L2data %>%
...
plot <- ggplot(filtered_data, aes(x = as.factor(Year), y = BIOMASS, fill = AGB_VEGTYPE)) +
  ...
  labs(
    ...
    title = "Annual variation in AGB understory at CZ-Lnz",
...
CZLnz_LAI$DATE2 <- ifelse(is.na(CZLnz_LAI$LAI_DATE), CZLnz_LAI$LAI_DATE_START, CZLnz_LAI$LAI_DATE)

#only data from CP - thus excluding SP-I DHPs from labelling
CZLnz_LAI <- CZLnz_LAI %>%
  filter(LAI_STATISTIC_NUMBER != "21")

CZLnz_LAI_mean <- CZLnz_LAI %>% filter(LAI_STATISTIC == "Mean")
CZLnz_LAI_sd <- CZLnz_LAI %>% filter(LAI_STATISTIC == "Standard Deviation")

combined_data <- CZLnz_LAI_mean %>%
  rename(Mean_LAI = LAI) %>%
  select(DATE2, Mean_LAI) %>%
  inner_join(CZLnz_LAI_sd %>% rename(SD_LAI = LAI) %>% select(DATE2, SD_LAI), by = "DATE2")
...
plot <- ggplot(combined_data, aes(x = DATE2)) +
  ...
  labs(x = "Date", y = "LAI ± SD (m²/m²)", title = "Seasonal variation in LAI at CZ-Lnz") +
  scale_x_date(
    date_labels = "%d-%m-%Y",
...
CZLnz_LITTER <- CZLnz_LITTER_L2data %>%
...
#CZLnz_LITTER_filtered <- CZLnz_LITTER %>%
#  filter(format(LITTER_DATE, "%Y") != "2022")



filtered_data <- CZLnz_LITTER %>% ####Double check: adjust if needed
...
combined_plot <- (plot_coarse + plot_fine_non) +
  plot_layout(ncol = 2) + 
  plot_annotation(
    title = "Mean (± SD) annual variation in dry litter mass at CZ-Lnz",  
    subtitle = "(n = number of CPs)",  
    theme = theme(
...
filtered_data2 <- CZLnz_LITTER %>% #### double check: delete _filtered
...
ggplot(filtered_data2, aes(x = Quarter, y = LITTER, fill = LITTER_ORGAN)) +
  geom_bar(stat = "identity", color = "black") +
  scale_fill_brewer(palette = "Accent") +
  labs(x = "Quarter", y = "Summed dry litter mass (kg/m²)", fill = "Litter Type", title = "Stacked non-woody litter per quarter at CZ-Lnz") +
  theme_minimal() +
  theme(
    legend.position = "bottom",
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, face = "bold"),
...
grid.text("\n Spatial variation in cumulative soil stock at CZ-Lnz",
...
CZLnz_VEG_CHEM_L2data <- read_latest_biftab("CZ-Lnz", data_dir)
CZLnz_VEG_CHEM_L2data <- CZLnz_VEG_CHEM_L2data %>%
  mutate(
    across(
...
plot_LMA <- function(df) {
  ...
  ggplot(combined_data, aes(x = DATE2, color = VEG_CHEM_SPP)) +
    ...
    labs(
      title = "Leaf Mass Area at CZ-Lnz",
      x = "Date",
      y = "LMA ± SD (kg/m²)",
      color = "Species"
    ) +
  ...
  plot_LMA(CZLnz_VEG_CHEM_L2data)
...
# Maak de plots
plot_C <- plot_conc(CZLnz_VEG_CHEM_L2data, "VEG_CHEM_CONC_C", "C concentration ± SD (gC/kg)")
plot_N <- plot_conc(CZLnz_VEG_CHEM_L2data, "VEG_CHEM_CONC_N", "N concentration ± SD (gN/kg)")


combined_plot <- (plot_C + plot_spacer() + plot_N) +
  plot_layout(ncol = 3, widths = c(1, 0.05, 1), guides = "collect") +
  plot_annotation(
    title = "Leaf nutrient analysis for C and N at CZ-Lnz",
    theme = theme(
      plot.title = element_text(face = "bold", size = 14, hjust = 0.5),
      legend.position = "none",
      legend.margin = margin(t = 30, b = 0, l = 0, r = 0)
...
library(dplyr)
 ...
 df <- read_phenocam("I:/Phenocam/CZ-Lnz-DU/ROI/CZ-Lnz-DU_DB_1009_3day.csv")
...
title(main = "Phenocam based phenology for Quercus robur L. at CZ-Lnz", font.main = 2, line = 4)
...
df <- read_phenocam("I:/Phenocam/CZ-Lnz-HA/ROI/CZ-Lnz-HA_DB_1009_3day.csv")
...
title(main = "Phenocam based phenology for Carpinus betulus L. at CZ-Lnz", font.main = 2, line = 4)
mtext("Date", side = 1, line = 2.5, font = 2)  
mtext("Green chromatic coordinate (Gcc_mean)", side = 2, line = 2.5, font = 2)  
...
text(
 ...
 df <- read_phenocam("I:/Phenocam/CZ-Lnz-JA/ROI/CZ-Lnz-JA_DB_1009_3day.csv")
...
title(main = "Phenocam based phenology for Fraxinus angustifolia Vahl at CZ-Lnz", font.main = 2, line = 4)
mtext("Date", side = 1, line = 2.5, font = 2)  
mtext("Green chromatic coordinate (Gcc_mean)", side = 2, line = 2.5, font = 2)  
...
- 3 different cameras with a focus on different species.

- Daily resolution = hourly between 10-16h (max 7 images/day).
