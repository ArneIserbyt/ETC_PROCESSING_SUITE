---
title: "<h1 style='font-weight:900;'>Summary Ancillary Data for DE-Tha</h1>
...
DETha_LAI <- safe_read_add_columns(path, "BIFTAB__GRP_LAI__ICOSETC_DE-Tha_ANCILLARY__.*\\.csv")
DETha_LITTER_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_LITTER__ICOSETC_DE-Tha_ANCILLARY__.*\\.csv")
DETha_BIOMASS_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BIOMASS__ICOSETC_DE-Tha_ANCILLARY__.*\\.csv")
DETha_DBH <- safe_read_add_columns(path, "BIFTAB__GRP_DBH__ICOSETC_DE-Tha_ANCILLARY__.*\\.csv")
DETha_HEIGHT <- safe_read_add_columns(path, "BIFTAB__GRP_HEIGHTC__ICOSETC_DE-Tha_ANCILLARY__.*\\.csv")
DETha_SPP_O_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_SPP_O__ICOSETC_DE-Tha_ANCILLARY__.*\\.csv")
DETha_TREES_NUM_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_TREES_NUM__ICOSETC_DE-Tha_ANCILLARY__.*\\.csv")
DETha_BASAL_AREA_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BASAL_AREA__ICOSETC_DE-Tha_ANCILLARY__.*\\.csv")


### Select the most common species (>5% of species contribution)
SPP_Threshold <- DETha_SPP_O_L2data %>%
...
TREEs_Site_CPs <- TREEs_Site %>%
  filter(
    grepl("^CP", TREE_PLOT) ) %>%
  mutate(Year = ifelse(Year %in% c("2019", "2020"), "2018", Year)) #unhashtag if needed for this site
...
DETha_Spatial <- read_excel("I:/ETC_ancillary data/Labelled/Aggregated data/Aggregations_L2_Forests/DATABASE_exports/GRP_PLOT_AllSites.xlsx")
DETha_Spatial2 <- DETha_Spatial %>% 
  filter(Site == "DE-Tha")
...
transformed_data <- DETha_BIOMASS_L2data %>%
...
ggplot(Pop_TREE_data_reduced2, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Biomass_TonsPerHa - Biomass_TonsPerHa_SD, 
                ...
                color = "black") +
  labs(
    title = "Standing (dry) biomass at DE-Tha",
...
transformed_data2 <- DETha_BASAL_AREA_L2data %>%
...
ggplot(Pop_TREE_data_reduced2ba, aes(x = TREE_SPP, y = BasalArea_m2Ha, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = BasalArea_m2Ha - BasalArea_m2Ha_SD, 
                ...
                color = "black") +
  labs(
    title = "Basal Area (m²/ha) at DE-Tha ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data3 <- DETha_TREES_NUM_L2data %>%
...
ggplot(Pop_TREE_data_reduced3, aes(x = TREE_SPP, y = Density_TreesPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Density_TreesPerHa - Density_TreesPerHa_SD, 
                ...
                color = "black") +
  labs(
      title = "Number of trees per hectare at DE-Tha ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
    y = "Tree density (#/ha)",
    fill = "Plot_Year"
...
DETha_SPP_O_L2data_filtered <- DETha_SPP_O_L2data %>%
  filter(
...
DETha_SPP_O_L2data_sorted <- DETha_SPP_O_L2data_filtered %>%
  arrange(Plot_Year, desc(SPP_O))

DETha_SPP_O_L2data_sorted <- DETha_SPP_O_L2data_sorted %>%
  group_by(Plot_Year) %>%
  mutate(SPP_O_SPP = factor(SPP_O_SPP, levels = unique(SPP_O_SPP[order(SPP_O)])))  

unique_species <- unique(DETha_SPP_O_L2data_sorted$SPP_O_SPP)
...
ggplot(DETha_SPP_O_L2data_sorted, aes(x = Plot_Year, y = SPP_O, fill = SPP_O_SPP)) +
  ...
  labs(
    title = "Average tree species ratio per plot/campaign at DE-Tha",
    # x = "Plot Year",
...
ggplot(Pop_TREE_data_dominant, aes(x = TREE_DBH_CM, fill = Year, group = Year)) +
  ...
  labs(
    title = "DBH distribution per observation year within CPs at DE-Tha",
...
ggplot() +
  ...
  labs(
    title = "Spatial variation in Biomass (tons/ha) at DE-Tha",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal() +
...
ggplot(Pop_TREE_data_reduced5, aes(x = TREE_SPP, y = DBH, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = DBH - DBH_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree DBH per species and plot/campaign at DE-Tha",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data4 <- DETha_HEIGHT %>%
...
ggplot(Pop_TREE_data_reduced4, aes(x = TREE_SPP, y = HEIGHTC, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = HEIGHTC - HEIGHTC_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree height per species and plot/campaign at DE-Tha",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
DETha_BIOMASS_DeadTrees <- DETha_BIOMASS_L2data %>% 
...
summarized_data <- DETha_BIOMASS_DeadTrees %>%
  group_by(TREE_SPP, Year, Plot, Plot_Year, BIOMASS_SPP, Species, BIOMASS_STATISTIC) %>%
  summarize(Biomass_TonsPerHa = mean(Biomass_TonsPerHa, na.rm = TRUE), .groups = 'drop')
...
ggplot(final_data, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  labs(
    title = "Biomass standing dead trees at DE-Tha",
    x = "", 
    y = "Biomass (tons/ha)",
    fill = "Plot_Year"
  ) +
...
filtered_data <- DETha_BIOMASS_L2data %>%
...
plot <- ggplot(filtered_data, aes(x = as.factor(Year), y = BIOMASS, fill = AGB_VEGTYPE)) +
  ...
  labs(
    ...
    title = "Annual variation in AGB understory at DE-Tha",
...
DETha_LAI$DATE2 <- ifelse(is.na(DETha_LAI$LAI_DATE), DETha_LAI$LAI_DATE_START, DETha_LAI$LAI_DATE)
...
DETha_LAI_mean <- DETha_LAI %>% filter(LAI_STATISTIC == "Mean")
DETha_LAI_sd <- DETha_LAI %>% filter(LAI_STATISTIC == "Standard Deviation")

combined_data <- DETha_LAI_mean %>%
  rename(Mean_LAI = LAI) %>%
  select(DATE2, Mean_LAI) %>%
  inner_join(DETha_LAI_sd %>% rename(SD_LAI = LAI) %>% select(DATE2, SD_LAI), by = "DATE2")
...
plot <- ggplot(combined_data, aes(x = DATE2)) +
  ...
  labs(x = "Date", y = "LAI ± SD (m²/m²)", title = "Seasonal variation in LAI at DE-Tha") +
  scale_x_date(
    date_labels = "%d-%m-%Y",
...
DETha_LITTER <- DETha_LITTER_L2data %>%
...
#DETha_LITTER_filtered <- DETha_LITTER %>%
#  filter(format(LITTER_DATE, "%Y") != "2022")



filtered_data <- DETha_LITTER %>% ####Double check: adjust if needed
...
combined_plot <- (plot_coarse + plot_fine_non) +
  plot_layout(ncol = 2) + 
  plot_annotation(
    title = "Mean (± SD) annual variation in dry litter mass at DE-Tha",  
    subtitle = "(n = number of CPs)",  
    theme = theme(
...
filtered_data2 <- DETha_LITTER %>% #### double check: delete _filtered
...
ggplot(filtered_data2, aes(x = Quarter, y = LITTER, fill = LITTER_ORGAN)) +
  geom_bar(stat = "identity", color = "black") +
  scale_fill_brewer(palette = "Accent") +
  labs(x = "Quarter", y = "Summed dry litter mass (kg/m²)", fill = "Litter Type", title = "Stacked non-woody litter per quarter at DE-Tha") +
  theme_minimal() +
  theme(
    legend.position = "bottom",
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, face = "bold"),
...
Soil data collection is currently in progress for DE-Tha
...
DETha_VEG_CHEM_L2data <- read_latest_biftab("DE-Tha", data_dir)
DETha_VEG_CHEM_L2data <- DETha_VEG_CHEM_L2data %>%
  mutate(
    across(
...
plot_LMA <- function(df) {
  ...
  ggplot(combined_data, aes(x = DATE2, color = VEG_CHEM_SPP)) +
    ...
    labs(
      title = "Leaf Mass Area at DE-Tha",
      x = "Date",
      y = "LMA ± SD (kg/m²)",
      color = "Species"
    ) +
  ...
  plot_LMA(DETha_VEG_CHEM_L2data)
...
# Maak de plots
plot_C <- plot_conc(DETha_VEG_CHEM_L2data, "VEG_CHEM_CONC_C", "C concentration ± SD (gC/kg)")
plot_N <- plot_conc(DETha_VEG_CHEM_L2data, "VEG_CHEM_CONC_N", "N concentration ± SD (gN/kg)")


combined_plot <- (plot_C + plot_spacer() + plot_N) +
  plot_layout(ncol = 3, widths = c(1, 0.05, 1), guides = "collect") +
  plot_annotation(
    title = "Leaf nutrient analysis for C and N at DE-Tha",
    theme = theme(
      plot.title = element_text(face = "bold", size = 14, hjust = 0.5),
...
<p style="font-size: 12px;">
ETC did not yet receive phenocam images for DE-Tha.
