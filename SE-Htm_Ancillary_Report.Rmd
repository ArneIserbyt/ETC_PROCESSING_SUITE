---
title: "<h1 style='font-weight:900;'>Summary Ancillary Data for SE-Htm</h1>
...
SEHtm_LAI <- safe_read_add_columns(path, "BIFTAB__GRP_LAI__ICOSETC_SE-Htm_ANCILLARY__.*\\.csv")
SEHtm_LITTER_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_LITTER__ICOSETC_SE-Htm_ANCILLARY__.*\\.csv")
SEHtm_BIOMASS_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BIOMASS__ICOSETC_SE-Htm_ANCILLARY__.*\\.csv")
SEHtm_DBH <- safe_read_add_columns(path, "BIFTAB__GRP_DBH__ICOSETC_SE-Htm_ANCILLARY__.*\\.csv")
SEHtm_HEIGHT <- safe_read_add_columns(path, "BIFTAB__GRP_HEIGHTC__ICOSETC_SE-Htm_ANCILLARY__.*\\.csv")
SEHtm_SPP_O_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_SPP_O__ICOSETC_SE-Htm_ANCILLARY__.*\\.csv")
SEHtm_TREES_NUM_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_TREES_NUM__ICOSETC_SE-Htm_ANCILLARY__.*\\.csv")
SEHtm_BASAL_AREA_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BASAL_AREA__ICOSETC_SE-Htm_ANCILLARY__.*\\.csv")


### Select the most common species (>5% of species contribution)
SPP_Threshold <- SEHtm_SPP_O_L2data %>%
...
SEHtm_Spatial <- read_excel("I:/ETC_ancillary data/Labelled/Aggregated data/Aggregations_L2_Forests/DATABASE_exports/GRP_PLOT_AllSites.xlsx")
SEHtm_Spatial2 <- SEHtm_Spatial %>% 
  filter(Site == "SE-Htm")
...
transformed_data <- SEHtm_BIOMASS_L2data %>%
...
ggplot(Pop_TREE_data_reduced2, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = PLOT_Year)) +
  ...
  geom_errorbar(aes(ymin = Biomass_TonsPerHa - Biomass_TonsPerHa_SD, 
                ...
                color = "black") +
  labs(
    title = "Standing (dry) biomass at SE-Htm",
...
transformed_data2 <- SEHtm_BASAL_AREA_L2data %>%
...
ggplot(Pop_TREE_data_reduced2ba, aes(x = TREE_SPP, y = BasalArea_m2Ha, fill = PLOT_Year)) +
  ...
  geom_errorbar(aes(ymin = BasalArea_m2Ha - BasalArea_m2Ha_SD, 
                ...
                color = "black") +
  labs(
    title = "Basal Area (m²/ha) at SE-Htm ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data3 <- SEHtm_TREES_NUM_L2data %>%
...
ggplot(Pop_TREE_data_reduced3, aes(x = TREE_SPP, y = Density_TreesPerHa, fill = PLOT_Year)) +
  ...
  geom_errorbar(aes(ymin = Density_TreesPerHa - Density_TreesPerHa_SD, 
                ...
                color = "black") +
  labs(
      title = "Number of trees per hectare at SE-Htm ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
    y = "Tree density (#/ha)",
    fill = "PLOT_Year"
...
SEHtm_SPP_O_L2data_filtered <- SEHtm_SPP_O_L2data %>%
  filter(
...
SEHtm_SPP_O_L2data_sorted <- SEHtm_SPP_O_L2data_filtered %>%
  arrange(PLOT_Year, desc(SPP_O))

SEHtm_SPP_O_L2data_sorted <- SEHtm_SPP_O_L2data_sorted %>%
  group_by(PLOT_Year) %>%
  mutate(SPP_O_SPP = factor(SPP_O_SPP, levels = unique(SPP_O_SPP[order(SPP_O)])))  

unique_species <- unique(SEHtm_SPP_O_L2data_sorted$SPP_O_SPP)
...
ggplot(SEHtm_SPP_O_L2data_sorted, aes(x = PLOT_Year, y = SPP_O, fill = SPP_O_SPP)) +
  ...
  labs(
    title = "Average tree species ratio per plot/campaign at SE-Htm",
    # x = "Plot Year",
...
ggplot(Pop_TREE_data_dominant, aes(x = TREE_DBH_CM, fill = Year, group = Year)) +
  ...
  labs(
    title = "DBH distribution per observation year within CPs at SE-Htm",
...
SEHtm_Spatial_filtered <- SEHtm_Spatial2 %>%
  select(PLOT_ID, PLOT_LOCATION_LAT, PLOT_LOCATION_LONG)
Biomass_PLOT <- Biomass_PLOT %>%
  left_join(SEHtm_Spatial_filtered, by = c("TREE_PLOT" = "PLOT_ID"))
...
ggplot() +
  ...
  labs(
    title = "Spatial variation in Biomass (tons/ha) at SE-Htm",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal() +
...
ggplot(Pop_TREE_data_reduced5, aes(x = TREE_SPP, y = DBH, fill = PLOT_Year)) +
  ...
  geom_errorbar(aes(ymin = DBH - DBH_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree DBH per species and plot/campaign at SE-Htm",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data4 <- SEHtm_HEIGHT %>%
...
ggplot(Pop_TREE_data_reduced4, aes(x = TREE_SPP, y = HEIGHTC, fill = PLOT_Year)) +
  ...
  geom_errorbar(aes(ymin = HEIGHTC - HEIGHTC_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree height per species and plot/campaign at SE-Htm",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
SEHtm_BIOMASS_DeadTrees <- SEHtm_BIOMASS_L2data %>% 
...
summarized_data <- SEHtm_BIOMASS_DeadTrees %>%
  group_by(TREE_SPP, Year, Plot, PLOT_Year, BIOMASS_SPP, Species, BIOMASS_STATISTIC) %>%
  summarize(Biomass_TonsPerHa = mean(Biomass_TonsPerHa, na.rm = TRUE), .groups = 'drop')
...
ggplot(final_data, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = PLOT_Year)) +
  ...
  labs(
    title = "Biomass standing dead trees at SE-Htm",
    x = "", 
    y = "Biomass (tons/ha)",
    fill = "PLOT_Year"
  ) +
...
filtered_data <- SEHtm_BIOMASS_L2data %>%
...
plot <- ggplot(filtered_data, aes(x = as.factor(Year), y = BIOMASS, fill = AGB_VEGTYPE)) +
  ...
  labs(
    ...
    title = "Annual variation in AGB understory at SE-Htm",
...
SEHtm_LAI$DATE2 <- ifelse(is.na(SEHtm_LAI$LAI_DATE), SEHtm_LAI$LAI_DATE_START, SEHtm_LAI$LAI_DATE)
...
SEHtm_LAI_mean <- SEHtm_LAI %>% filter(LAI_STATISTIC == "Mean")
SEHtm_LAI_sd <- SEHtm_LAI %>% filter(LAI_STATISTIC == "Standard Deviation")

combined_data <- SEHtm_LAI_mean %>%
  rename(Mean_LAI = LAI) %>%
  select(DATE2, Mean_LAI) %>%
  inner_join(SEHtm_LAI_sd %>% rename(SD_LAI = LAI) %>% select(DATE2, SD_LAI), by = "DATE2")
...
plot <- ggplot(combined_data, aes(x = DATE2)) +
  ...
  labs(x = "Date", y = "LAI ± SD (m²/m²)", title = "Seasonal variation in LAI at SE-Htm") +
  scale_x_date(
    date_labels = "%d-%m-%Y",
...
SEHtm_LITTER <- SEHtm_LITTER_L2data %>%
...
#SEHtm_LITTER_filtered <- SEHtm_LITTER %>%
#  filter(format(LITTER_DATE, "%Y") != "2022")



filtered_data <- SEHtm_LITTER %>% ####Double check: adjust if needed
...
combined_plot <- (plot_coarse + plot_fine_non) +
  plot_layout(ncol = 2) + 
  plot_annotation(
    title = "Mean (± SD) annual variation in dry litter mass at SE-Htm",  
    subtitle = "(n = number of CPs)",  
    theme = theme(
...
* Specific note for SE-Htm: The high value for non-woody litter in 2022-Q1 is driven by the prevailing weather conditions at the time.
...
filtered_data2 <- SEHtm_LITTER %>% #### double check: delete _filtered
...
ggplot(filtered_data2, aes(x = Quarter, y = LITTER, fill = LITTER_ORGAN)) +
  geom_bar(stat = "identity", color = "black") +
  scale_fill_brewer(palette = "Accent") +
  labs(x = "Quarter", y = "Summed dry litter mass (kg/m²)", fill = "Litter Type", title = "Stacked non-woody litter per quarter at SE-Htm") +
  theme_minimal() +
  theme(
    legend.position = "bottom",
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, face = "bold"),
...
grid.text("\n Spatial variation in cumulative soil stock at SE-Htm",
...
SEHtm_VEG_CHEM_L2data <- read_latest_biftab("SE-Htm", data_dir)
SEHtm_VEG_CHEM_L2data <- SEHtm_VEG_CHEM_L2data %>%
  mutate(
    across(
...
plot_LMA <- function(df) {
  ...
  ggplot(combined_data, aes(x = DATE2, color = VEG_CHEM_SPP)) +
    ...
    labs(
      title = "Leaf Mass Area at SE-Htm",
      x = "Date",
      y = "LMA ± SD (kg/m²)",
      color = "Species"
    ) +
  ...
  plot_LMA(SEHtm_VEG_CHEM_L2data)
...
# Maak de plots
plot_C <- plot_conc(SEHtm_VEG_CHEM_L2data, "VEG_CHEM_CONC_C", "C concentration ± SD (gC/kg)")
plot_N <- plot_conc(SEHtm_VEG_CHEM_L2data, "VEG_CHEM_CONC_N", "N concentration ± SD (gN/kg)")


combined_plot <- (plot_C + plot_spacer() + plot_N) +
  plot_layout(ncol = 3, widths = c(1, 0.05, 1), guides = "collect") +
  plot_annotation(
    title = "Leaf nutrient analysis for C and N at SE-Htm",
    theme = theme(
      plot.title = element_text(face = "bold", size = 14, hjust = 0.5),
      legend.position = "none",
      legend.margin = margin(t = 30, b = 0, l = 0, r = 0)
...
df <- read_phenocam("I:/Phenocam/dailyResults/SE-Htm-L17-F01_DB_1000_3day.csv")

df <- expand_phenocam(df)
df <- detect_outliers(df)
df <- smooth_ts(df)

transition_dates_obj <- transition_dates(
  df,
  percentile = 0.8
)
...
title(
  main = "Phenocam based phenology for SE-Htm",
  font.main = 2,
  line = 4
)
...
if (length(all_dates) > 0) {
  ...
  message("Geen phenologische overgangsdata gevonden voor gcc_mean.")
}
...
```{r PHENOCAM understory, fig.width=10, fig.height=6, dpi=300}

df <- read_phenocam("I:/Phenocam/dailyResults/SE-Htm-L18-F02_DB_1000_3day.csv")
...
title(
  main = "Phenocam based phenology for SE-Htm (below canopy)",
  font.main = 2,
  line = 4
)
