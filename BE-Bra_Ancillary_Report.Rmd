---
title: "<h1 style='font-weight:900;'>Summary Ancillary Data for BE-Bra</h1>
...
BEBra_LAI <- safe_read_add_columns(path, "BIFTAB__GRP_LAI__ICOSETC_BE-Bra_ANCILLARY__.*\\.csv")
BEBra_LITTER_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_LITTER__ICOSETC_BE-Bra_ANCILLARY__.*\\.csv")
BEBra_BIOMASS_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BIOMASS__ICOSETC_BE-Bra_ANCILLARY__.*\\.csv")
BEBra_DBH <- safe_read_add_columns(path, "BIFTAB__GRP_DBH__ICOSETC_BE-Bra_ANCILLARY__.*\\.csv")
BEBra_HEIGHT <- safe_read_add_columns(path, "BIFTAB__GRP_HEIGHTC__ICOSETC_BE-Bra_ANCILLARY__.*\\.csv")
BEBra_SPP_O_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_SPP_O__ICOSETC_BE-Bra_ANCILLARY__.*\\.csv")
BEBra_TREES_NUM_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_TREES_NUM__ICOSETC_BE-Bra_ANCILLARY__.*\\.csv")
BEBra_BASAL_AREA_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BASAL_AREA__ICOSETC_BE-Bra_ANCILLARY__.*\\.csv")


### load FLSM

### Select the most common species (>5% of species contribution)
SPP_Threshold <- BEBra_SPP_O_L2data %>%
...
TREEs_Site_CPs <- TREEs_Site %>%
  filter(
    grepl("^CP", TREE_PLOT) ) %>%
  mutate(Year = ifelse(Year == "2021", "2020", Year)) #unhashtag if needed for this site
...
BEBra_Spatial <- read_excel("I:/ETC_ancillary data/Labelled/Aggregated data/Aggregations_L2_Forests/DATABASE_exports/GRP_PLOT_AllSites.xlsx")
BEBra_Spatial2 <- BEBra_Spatial %>% 
  filter(Site == "BE-Bra")
...
transformed_data <- BEBra_BIOMASS_L2data %>%
...
ggplot(Pop_TREE_data_reduced2, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Biomass_TonsPerHa - Biomass_TonsPerHa_SD, 
                ...
                color = "black") +
  labs(
    title = "Standing (dry) biomass at BE-Bra",
...
transformed_data2 <- BEBra_BASAL_AREA_L2data %>%
...
ggplot(Pop_TREE_data_reduced2ba, aes(x = TREE_SPP, y = BasalArea_m2Ha, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = BasalArea_m2Ha - BasalArea_m2Ha_SD, 
                ...
                color = "black") +
  labs(
    title = "Basal Area (m²/ha) at BE-Bra ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data3 <- BEBra_TREES_NUM_L2data %>%
...
ggplot(Pop_TREE_data_reduced3, aes(x = TREE_SPP, y = Density_TreesPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Density_TreesPerHa - Density_TreesPerHa_SD, 
                ...
                color = "black") +
  labs(
      title = "Number of trees per hectare at BE-Bra ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
    y = "Tree density (#/ha)",
    fill = "Plot_Year"
...
BEBra_SPP_O_L2data_filtered <- BEBra_SPP_O_L2data %>%
  filter(
...
BEBra_SPP_O_L2data_sorted <- BEBra_SPP_O_L2data_filtered %>%
  arrange(Plot_Year, desc(SPP_O))

BEBra_SPP_O_L2data_sorted <- BEBra_SPP_O_L2data_sorted %>%
  group_by(Plot_Year) %>%
  mutate(SPP_O_SPP = factor(SPP_O_SPP, levels = unique(SPP_O_SPP[order(SPP_O)])))  

unique_species <- unique(BEBra_SPP_O_L2data_sorted$SPP_O_SPP)
...
ggplot(BEBra_SPP_O_L2data_sorted, aes(x = Plot_Year, y = SPP_O, fill = SPP_O_SPP)) +
  ...
  labs(
    title = "Average tree species ratio per plot/campaign at BE-Bra",
    # x = "Plot Year",
...
ggplot(Pop_TREE_data_dominant, aes(x = TREE_DBH_CM, fill = Year, group = Year)) +
  ...
  labs(
    title = "DBH distribution per observation year within CPs at BE-Bra",
...
BEBra_Spatial_filtered <- BEBra_Spatial2 %>%
  select(PLOT_ID, PLOT_LOCATION_LAT, PLOT_LOCATION_LONG)
Biomass_PLOT <- Biomass_PLOT %>%
  left_join(BEBra_Spatial_filtered, by = c("TREE_PLOT" = "PLOT_ID"))
...
ggplot() +
  ...
  labs(
    title = "Spatial variation in Biomass (tons/ha) at BE-Bra",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal() +
...
ggplot(Pop_TREE_data_reduced5, aes(x = TREE_SPP, y = DBH, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = DBH - DBH_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree DBH per species and plot/campaign at BE-Bra",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data4 <- BEBra_HEIGHT %>%
...
ggplot(Pop_TREE_data_reduced4, aes(x = TREE_SPP, y = HEIGHTC, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = HEIGHTC - HEIGHTC_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree height per species and plot/campaign at BE-Bra",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
BEBra_BIOMASS_DeadTrees <- BEBra_BIOMASS_L2data %>% 
...
summarized_data <- BEBra_BIOMASS_DeadTrees %>%
  group_by(TREE_SPP, Year, Plot, Plot_Year, BIOMASS_SPP, Species, BIOMASS_STATISTIC) %>%
  summarize(Biomass_TonsPerHa = mean(Biomass_TonsPerHa, na.rm = TRUE), .groups = 'drop')
...
ggplot(final_data, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  labs(
    title = "Biomass standing dead trees at BE-Bra",
    x = "", 
    y = "Biomass (tons/ha)",
    fill = "Plot_Year"
  ) +
...
filtered_data <- BEBra_BIOMASS_L2data %>%
...
plot <- ggplot(filtered_data, aes(x = as.factor(Year), y = BIOMASS, fill = AGB_VEGTYPE)) +
  ...
  labs(
    ...
    title = "Annual variation in AGB understory at BE-Bra",
...
BEBra_LAI$DATE2 <- ifelse(is.na(BEBra_LAI$LAI_DATE), BEBra_LAI$LAI_DATE_START, BEBra_LAI$LAI_DATE)
...
BEBra_LAI_mean <- BEBra_LAI %>% filter(LAI_STATISTIC == "Mean")
BEBra_LAI_sd <- BEBra_LAI %>% filter(LAI_STATISTIC == "Standard Deviation")

combined_data <- BEBra_LAI_mean %>%
  rename(Mean_LAI = LAI) %>%
  select(DATE2, Mean_LAI) %>%
  inner_join(BEBra_LAI_sd %>% rename(SD_LAI = LAI) %>% select(DATE2, SD_LAI), by = "DATE2")
...
plot <- ggplot(combined_data, aes(x = DATE2)) +
  ...
  labs(x = "Date", y = "LAI ± SD (m²/m²)", title = "Seasonal variation in LAI at BE-Bra") +
  scale_x_date(
    date_labels = "%d-%m-%Y",
...
BEBra_LITTER <- BEBra_LITTER_L2data %>%
...
#BEBra_LITTER_filtered <- BEBra_LITTER %>%
#  filter(format(LITTER_DATE, "%Y") != "2022")



filtered_data <- BEBra_LITTER %>% ####Double check: adjust if needed
...
combined_plot <- (plot_coarse + plot_fine_non) +
  plot_layout(ncol = 2) + 
  plot_annotation(
    title = "Mean (± SD) annual variation in dry litter mass at BE-Bra",  
    subtitle = "(n = number of CPs)",  
    theme = theme(
...
filtered_data2 <- BEBra_LITTER %>% #### double check: delete _filtered
...
ggplot(filtered_data2, aes(x = Quarter, y = LITTER, fill = LITTER_ORGAN)) +
  geom_bar(stat = "identity", color = "black") +
  scale_fill_brewer(palette = "Accent") +
  labs(x = "Quarter", y = "Summed dry litter mass (kg/m²)", fill = "Litter Type", title = "Stacked non-woody litter per quarter at BE-Bra") +
  theme_minimal() +
  theme(
    legend.position = "bottom",
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, face = "bold"),
...
grid.text(
  paste0("\nSpatial variation in cumulative soil stock at ", MySite),
  x = 0.5,
  y = 0.99,
...
BEBra_VEG_CHEM_L2data <- read_latest_biftab("BE-Bra", data_dir)
BEBra_VEG_CHEM_L2data <- BEBra_VEG_CHEM_L2data %>%
  mutate(
    across(
...
plot_LMA <- function(df) {
  ...
  ggplot(combined_data, aes(x = DATE2, color = VEG_CHEM_SPP)) +
    ...
    labs(
      title = "Leaf Mass Area at BE-Bra",
      x = "Date",
      y = "LMA ± SD (kg/m²)",
      color = "Species"
    ) +
  ...
  plot_LMA(BEBra_VEG_CHEM_L2data)
...
# Maak de plots
plot_C <- plot_conc(BEBra_VEG_CHEM_L2data, "VEG_CHEM_CONC_C", "C concentration ± SD (gC/kg)")
plot_N <- plot_conc(BEBra_VEG_CHEM_L2data, "VEG_CHEM_CONC_N", "N concentration ± SD (gN/kg)")


combined_plot <- (plot_C + plot_spacer() + plot_N) +
  plot_layout(ncol = 3, widths = c(1, 0.05, 1), guides = "collect") +
  plot_annotation(
    title = "Leaf nutrient analysis for C and N at BE-Bra",
    theme = theme(
      plot.title = element_text(face = "bold", size = 14, hjust = 0.5),
      legend.position = "none",
      legend.margin = margin(t = 30, b = 0, l = 0, r = 0)
...
```{r PHENOCAM, fig.width=10, fig.height=6, dpi=300}
 library(phenocamr)
df <- read_phenocam("I:/Phenocam/BE-Bra/ROI/BE-Bra_DB_1000_3day.csv")

df <- expand_phenocam(df)
df <- detect_outliers(df)
df <- smooth_ts(df)
transition_dates <- transition_dates(df, percentile = 0.8 )
...
title(main = "Phenocam based phenology at BE-Bra", font.main = 2, line = 4)
...
<p style="font-size: 12px;">
<u> Notes for BE-Bra: </u>

- Camera angle until 17/06/2019 - 10H30: mainly canopy, no horizon

- Camera angle between 17/06/2019 - 10H30 and 17/08/2022 - 8h00 (slightly more downwards and more zoomed in). 

- From 17/08/2022 - 8h00 onwards complient with ICOS instructions (including horizon).

- ROI is always adjusted and thus the mask includes always the same selected trees.However, the zooming has an impact on the number of pixels within the ROI, which then may have a (slight) impact on calculated Gcc.

- Resolution until 17/8/2022 = hourly between 4am and 22 pm (17 images/day)

- Resolution after 17/8/2022 = every 15 minutes between 4am and 22 pm (72 images/day).

- <b>MOST IMPORTANT:</b> Camera white balance was not fixed, which most likely explains the data jump in 2023-2024.

- <b>Ad hoc solution:</b> between 17/08/2022 and 05/11/2024, the Gcc_mean values were decreased with 0.01305 and after 05/11/2024 these values were raised with 0.01305.

</p>



```{r PHENOCAM2, fig.width=10, fig.height=6, dpi=300}
...
# Zorg dat de kolom date een Date is
df$data$date <- as.Date(df$data$date)

# Definieer grenzen
start_date <- as.Date("2022-08-17")
end_date   <- as.Date("2024-11-05")

df$data$gcc_mean[df$data$date >= start_date & df$data$date <= end_date] <- 
  df$data$gcc_mean[df$data$date >= start_date & df$data$date <= end_date] - 0.01305

df$data$gcc_mean[df$data$date > end_date] <- 
  df$data$gcc_mean[df$data$date > end_date] + 0.01305
...
title(main = "Phenocam based phenology at BE-Bra (correction)", font.main = 2, line = 4)
mtext("Date", side = 1, line = 2.5, font = 2)  
mtext("Green chromatic coordinate (Gcc_mean)", side = 2, line = 2.5, font = 2)  
