---
title: "<h1 style='font-weight:900;'>Summary Ancillary Data for BE-Vie</h1>
...
BEVie_LAI <- safe_read_add_columns(path, "BIFTAB__GRP_LAI__ICOSETC_BE-Vie_ANCILLARY__.*\\.csv")
BEVie_LITTER_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_LITTER__ICOSETC_BE-Vie_ANCILLARY__.*\\.csv")
BEVie_BIOMASS_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BIOMASS__ICOSETC_BE-Vie_ANCILLARY__.*\\.csv")
BEVie_DBH <- safe_read_add_columns(path, "BIFTAB__GRP_DBH__ICOSETC_BE-Vie_ANCILLARY__.*\\.csv")
BEVie_HEIGHT <- safe_read_add_columns(path, "BIFTAB__GRP_HEIGHTC__ICOSETC_BE-Vie_ANCILLARY__.*\\.csv")
BEVie_SPP_O_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_SPP_O__ICOSETC_BE-Vie_ANCILLARY__.*\\.csv")
BEVie_TREES_NUM_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_TREES_NUM__ICOSETC_BE-Vie_ANCILLARY__.*\\.csv")
BEVie_BASAL_AREA_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BASAL_AREA__ICOSETC_BE-Vie_ANCILLARY__.*\\.csv")


### Select the most common species (>5% of species contribution)
SPP_Threshold <- BEVie_SPP_O_L2data %>%
...
BEVie_Spatial <- read_excel("I:/ETC_ancillary data/Labelled/Aggregated data/Aggregations_L2_Forests/DATABASE_exports/GRP_PLOT_AllSites.xlsx")
BEVie_Spatial2 <- BEVie_Spatial %>% 
  filter(Site == "BE-Vie")
...
transformed_data <- BEVie_BIOMASS_L2data %>%
...
ggplot(Pop_TREE_data_reduced2, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Biomass_TonsPerHa - Biomass_TonsPerHa_SD, 
                ...
                color = "black") +
  labs(
    title = "Standing (dry) biomass at BE-Vie",
...
transformed_data2 <- BEVie_BASAL_AREA_L2data %>%
...
ggplot(Pop_TREE_data_reduced2ba, aes(x = TREE_SPP, y = BasalArea_m2Ha, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = BasalArea_m2Ha - BasalArea_m2Ha_SD, 
                ...
                color = "black") +
  labs(
    title = "Basal Area (m²/ha) at BE-Vie ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data3 <- BEVie_TREES_NUM_L2data %>%
...
ggplot(Pop_TREE_data_reduced3, aes(x = TREE_SPP, y = Density_TreesPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Density_TreesPerHa - Density_TreesPerHa_SD, 
                ...
                color = "black") +
  labs(
      title = "Number of trees per hectare at BE-Vie ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
    y = "Tree density (#/ha)",
    fill = "Plot_Year"
...
BEVie_SPP_O_L2data_filtered <- BEVie_SPP_O_L2data %>%
  filter(
...
BEVie_SPP_O_L2data_sorted <- BEVie_SPP_O_L2data_filtered %>%
  arrange(Plot_Year, desc(SPP_O))

BEVie_SPP_O_L2data_sorted <- BEVie_SPP_O_L2data_sorted %>%
  group_by(Plot_Year) %>%
  mutate(SPP_O_SPP = factor(SPP_O_SPP, levels = unique(SPP_O_SPP[order(SPP_O)])))  

unique_species <- unique(BEVie_SPP_O_L2data_sorted$SPP_O_SPP)
...
ggplot(BEVie_SPP_O_L2data_sorted, aes(x = Plot_Year, y = SPP_O, fill = SPP_O_SPP)) +
  ...
  labs(
    title = "Average tree species ratio per plot/campaign at BE-Vie",
    # x = "Plot Year",
...
ggplot(Pop_TREE_data_dominant, aes(x = TREE_DBH_CM, fill = Year, group = Year)) +
  ...
  labs(
    title = "DBH distribution per observation year within CPs at BE-Vie",
...
BEVie_Spatial_filtered <- BEVie_Spatial2 %>%
  select(PLOT_ID, PLOT_LOCATION_LAT, PLOT_LOCATION_LONG)
Biomass_PLOT <- Biomass_PLOT %>%
  left_join(BEVie_Spatial_filtered, by = c("TREE_PLOT" = "PLOT_ID"))
...
ggplot() +
  ...
  labs(
    title = "Spatial variation in Biomass (tons/ha) at BE-Vie",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal() +
...
ggplot(Pop_TREE_data_reduced5, aes(x = TREE_SPP, y = DBH, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = DBH - DBH_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree DBH per species and plot/campaign at BE-Vie",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data4 <- BEVie_HEIGHT %>%
...
ggplot(Pop_TREE_data_reduced4, aes(x = TREE_SPP, y = HEIGHTC, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = HEIGHTC - HEIGHTC_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree height per species and plot/campaign at BE-Vie",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
BEVie_BIOMASS_DeadTrees <- BEVie_BIOMASS_L2data %>% 
...
summarized_data <- BEVie_BIOMASS_DeadTrees %>%
  group_by(TREE_SPP, Year, Plot, Plot_Year, BIOMASS_SPP, Species, BIOMASS_STATISTIC) %>%
  summarize(Biomass_TonsPerHa = mean(Biomass_TonsPerHa, na.rm = TRUE), .groups = 'drop')
...
ggplot(final_data, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  labs(
    title = "Biomass standing dead trees at BE-Vie",
    x = "", 
    y = "Biomass (tons/ha)",
    fill = "Plot_Year"
  ) +
...
filtered_data <- BEVie_BIOMASS_L2data %>%
...
plot <- ggplot(filtered_data, aes(x = as.factor(Year), y = BIOMASS, fill = AGB_VEGTYPE)) +
  ...
  labs(
    ...
    title = "Annual variation in AGB understory at BE-Vie",
...
BEVie_LAI$DATE2 <- ifelse(is.na(BEVie_LAI$LAI_DATE), BEVie_LAI$LAI_DATE_START, BEVie_LAI$LAI_DATE)
...
BEVie_LAI_mean <- BEVie_LAI %>% filter(LAI_STATISTIC == "Mean")
BEVie_LAI_sd <- BEVie_LAI %>% filter(LAI_STATISTIC == "Standard Deviation")

combined_data <- BEVie_LAI_mean %>%
  rename(Mean_LAI = LAI) %>%
  select(DATE2, Mean_LAI) %>%
  inner_join(BEVie_LAI_sd %>% rename(SD_LAI = LAI) %>% select(DATE2, SD_LAI), by = "DATE2")
...
plot <- ggplot(combined_data, aes(x = DATE2)) +
  ...
  labs(x = "Date", y = "LAI ± SD (m²/m²)", title = "Seasonal variation in LAI at BE-Vie") +
  scale_x_date(
    date_labels = "%d-%m-%Y",
...
<p style="font-size: 12px;">
BE-Vie is a Class2 site, hence litter is not collected.
...
grid.text("\n Spatial variation in cumulative soil stock at BE-Vie",
...
BEVie_VEG_CHEM_L2data <- read_latest_biftab("BE-Vie", data_dir)
BEVie_VEG_CHEM_L2data <- BEVie_VEG_CHEM_L2data %>%
  mutate(
    across(
...
plot_LMA <- function(df) {
  ...
  ggplot(combined_data, aes(x = DATE2, color = VEG_CHEM_SPP)) +
    ...
    labs(
      title = "Leaf Mass Area at BE-Vie",
      x = "Date",
      y = "LMA ± SD (kg/m²)",
      color = "Species"
    ) +
  ...
  plot_LMA(BEVie_VEG_CHEM_L2data)
...
# Maak de plots
plot_C <- plot_conc(BEVie_VEG_CHEM_L2data, "VEG_CHEM_CONC_C", "C concentration ± SD (gC/kg)")
plot_N <- plot_conc(BEVie_VEG_CHEM_L2data, "VEG_CHEM_CONC_N", "N concentration ± SD (gN/kg)")


combined_plot <- (plot_C + plot_spacer() + plot_N) +
  plot_layout(ncol = 3, widths = c(1, 0.05, 1), guides = "collect") +
  plot_annotation(
    title = "Leaf nutrient analysis for C and N at BE-Vie",
    theme = theme(
      plot.title = element_text(face = "bold", size = 14, hjust = 0.5),
      legend.position = "none",
      legend.margin = margin(t = 30, b = 0, l = 0, r = 0)
...
```{r PHENOCAM, fig.width=10, fig.height=6, dpi=300}
 library(phenocamr)
df <- read_phenocam("I:/Phenocam/BE-Vie/ROI/BE-Vie_DB_1003_3day.csv")
...
title(main = "Phenocam based phenology at BE-Vie", font.main = 2, line = 4)
...
- Daily resolution from 2015: 4 images/day: 11h, 12h, 13h and 15h.
- Daily resolution from 2023 onwards: 45 images/day: half-hourly for 24h.
- Data gap in autumn 2024 (2 months).
<br><br>
