---
title: "<h1 style='font-weight:900;'>Summary Ancillary Data for FI-Sod</h1>
...
FISod_LAI <- safe_read_add_columns(path, "BIFTAB__GRP_LAI__ICOSETC_FI-Sod_ANCILLARY__.*\\.csv")
FISod_LITTER_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_LITTER__ICOSETC_FI-Sod_ANCILLARY__.*\\.csv")
FISod_BIOMASS_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BIOMASS__ICOSETC_FI-Sod_ANCILLARY__.*\\.csv")
FISod_DBH <- safe_read_add_columns(path, "BIFTAB__GRP_DBH__ICOSETC_FI-Sod_ANCILLARY__.*\\.csv")
FISod_HEIGHT <- safe_read_add_columns(path, "BIFTAB__GRP_HEIGHTC__ICOSETC_FI-Sod_ANCILLARY__.*\\.csv")
FISod_SPP_O_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_SPP_O__ICOSETC_FI-Sod_ANCILLARY__.*\\.csv")
FISod_TREES_NUM_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_TREES_NUM__ICOSETC_FI-Sod_ANCILLARY__.*\\.csv")
FISod_BASAL_AREA_L2data <- safe_read_add_columns(path, "BIFTAB__GRP_BASAL_AREA__ICOSETC_FI-Sod_ANCILLARY__.*\\.csv")


### Select the most common species (>5% of species contribution)
SPP_Threshold <- FISod_SPP_O_L2data %>%
...
TREEs_Site_CPs <- TREEs_Site %>%
  filter(
    grepl("^CP", TREE_PLOT) )  %>%
  mutate(Year = ifelse(Year == "2019", "2018", Year)) #unhashtag if needed for this site
...
FISod_Spatial <- read_excel("I:/ETC_ancillary data/Labelled/Aggregated data/Aggregations_L2_Forests/DATABASE_exports/GRP_PLOT_AllSites.xlsx")
FISod_Spatial2 <- FISod_Spatial %>% 
  filter(Site == "FI-Sod")
...
transformed_data <- FISod_BIOMASS_L2data %>%
...
ggplot(Pop_TREE_data_reduced2, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Biomass_TonsPerHa - Biomass_TonsPerHa_SD, 
                ...
                color = "black") +
  labs(
    title = "Standing (dry) biomass at FI-Sod",
...
transformed_data2 <- FISod_BASAL_AREA_L2data %>%
...
ggplot(Pop_TREE_data_reduced2ba, aes(x = TREE_SPP, y = BasalArea_m2Ha, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = BasalArea_m2Ha - BasalArea_m2Ha_SD, 
                ...
                color = "black") +
  labs(
    title = "Basal Area (m²/ha) at FI-Sod ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data3 <- FISod_TREES_NUM_L2data %>%
...
ggplot(Pop_TREE_data_reduced3, aes(x = TREE_SPP, y = Density_TreesPerHa, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = Density_TreesPerHa - Density_TreesPerHa_SD, 
                ...
                color = "black") +
  labs(
      title = "Number of trees per hectare at FI-Sod ",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
    y = "Tree density (#/ha)",
    fill = "Plot_Year"
...
FISod_SPP_O_L2data_filtered <- FISod_SPP_O_L2data %>%
  filter(
...
FISod_SPP_O_L2data_sorted <- FISod_SPP_O_L2data_filtered %>%
  arrange(Plot_Year, desc(SPP_O))

FISod_SPP_O_L2data_sorted <- FISod_SPP_O_L2data_sorted %>%
  group_by(Plot_Year) %>%
  mutate(SPP_O_SPP = factor(SPP_O_SPP, levels = unique(SPP_O_SPP[order(SPP_O)])))  

unique_species <- unique(FISod_SPP_O_L2data_sorted$SPP_O_SPP)
...
ggplot(FISod_SPP_O_L2data_sorted, aes(x = Plot_Year, y = SPP_O, fill = SPP_O_SPP)) +
  ...
  labs(
    title = "Average tree species ratio per plot/campaign at FI-Sod",
    # x = "Plot Year",
...
ggplot(Pop_TREE_data_dominant, aes(x = TREE_DBH_CM, fill = Year, group = Year)) +
  ...
  labs(
    title = "DBH distribution per observation year within CPs at FI-Sod",
...
FISod_Spatial_filtered <- FISod_Spatial2 %>%
  select(PLOT_ID, PLOT_LOCATION_LAT, PLOT_LOCATION_LONG)
Biomass_PLOT <- Biomass_PLOT %>%
  left_join(FISod_Spatial_filtered, by = c("TREE_PLOT" = "PLOT_ID"))
...
ggplot() +
  ...
  labs(
    title = "Spatial variation in Biomass (tons/ha) at FI-Sod",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal() +
...
ggplot(Pop_TREE_data_reduced5, aes(x = TREE_SPP, y = DBH, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = DBH - DBH_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree DBH per species and plot/campaign at FI-Sod",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
transformed_data4 <- FISod_HEIGHT %>%
...
ggplot(Pop_TREE_data_reduced4, aes(x = TREE_SPP, y = HEIGHTC, fill = Plot_Year)) +
  ...
  geom_errorbar(aes(ymin = HEIGHTC - HEIGHTC_SD, 
                ...
                color = "black") +
  labs(
    title = "Average tree height per species and plot/campaign at FI-Sod",
    subtitle = "only includes species that contribute >5% to the total tree count",  
    x = "Species",
...
FISod_BIOMASS_DeadTrees <- FISod_BIOMASS_L2data %>% 
...
summarized_data <- FISod_BIOMASS_DeadTrees %>%
  group_by(TREE_SPP, Year, Plot, Plot_Year, BIOMASS_SPP, Species, BIOMASS_STATISTIC) %>%
  summarize(Biomass_TonsPerHa = mean(Biomass_TonsPerHa, na.rm = TRUE), .groups = 'drop')
...
ggplot(final_data, aes(x = TREE_SPP, y = Biomass_TonsPerHa, fill = Plot_Year)) +
  ...
  labs(
    title = "Biomass standing dead trees at FI-Sod",
    x = "", 
    y = "Biomass (tons/ha)",
    fill = "Plot_Year"
  ) +
...
filtered_data <- FISod_BIOMASS_L2data %>%
...
plot <- ggplot(filtered_data, aes(x = as.factor(Year), y = BIOMASS, fill = AGB_VEGTYPE)) +
  ...
  labs(
    ...
    title = "Annual variation in AGB understory at FI-Sod",
...
<br>
GAI data compilation is currently in progress for FI-Sod.

</p>


```{r GRP_GAI, eval=FALSE, fig.width=10, fig.height=6, dpi=300}

FISod_LAI$DATE2 <- ifelse(is.na(FISod_LAI$LAI_DATE), FISod_LAI$LAI_DATE_START, FISod_LAI$LAI_DATE)
...
FISod_LAI_mean <- FISod_LAI %>% filter(LAI_STATISTIC == "Mean")
FISod_LAI_sd <- FISod_LAI %>% filter(LAI_STATISTIC == "Standard Deviation")

combined_data <- FISod_LAI_mean %>%
  rename(Mean_LAI = LAI) %>%
  select(DATE2, Mean_LAI) %>%
  inner_join(FISod_LAI_sd %>% rename(SD_LAI = LAI) %>% select(DATE2, SD_LAI), by = "DATE2")
...
plot <- ggplot(combined_data, aes(x = DATE2)) +
  ...
  labs(x = "Date", y = "LAI ± SD (m²/m²)", title = "Seasonal variation in LAI at FI-Sod") +
  scale_x_date(
    date_labels = "%d-%m-%Y",
...
FISod_LITTER <- FISod_LITTER_L2data %>%
...
#FISod_LITTER_filtered <- FISod_LITTER %>%
#  filter(format(LITTER_DATE, "%Y") != "2022")



filtered_data <- FISod_LITTER %>% ####Double check: adjust if needed
...
combined_plot <- (plot_coarse + plot_fine_non) +
  plot_layout(ncol = 2) + 
  plot_annotation(
    title = "Mean (± SD) annual variation in dry litter mass at FI-Sod",  
    subtitle = "(n = number of CPs)",  
    theme = theme(
...
filtered_data2 <- FISod_LITTER %>% #### double check: delete _filtered
...
ggplot(filtered_data2, aes(x = Quarter, y = LITTER, fill = LITTER_ORGAN)) +
  geom_bar(stat = "identity", color = "black") +
  scale_fill_brewer(palette = "Accent") +
  labs(x = "Quarter", y = "Summed dry litter mass (kg/m²)", fill = "Litter Type", title = "Stacked non-woody litter per quarter at FI-Sod") +
  theme_minimal() +
  theme(
    legend.position = "bottom",
    axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, face = "bold"),
...
Soil data collection is currently in progress for FI-Sod.
...
FISod_VEG_CHEM_L2data <- read_latest_biftab("FI-Sod", data_dir)
FISod_VEG_CHEM_L2data <- FISod_VEG_CHEM_L2data %>%
  mutate(
    across(
...
plot_LMA <- function(df) {
  ...
  ggplot(combined_data, aes(x = DATE2, color = VEG_CHEM_SPP)) +
    ...
    labs(
      title = "Leaf Mass Area at FI-Sod",
      x = "Date",
      y = "LMA ± SD (kg/m²)",
      color = "Species"
    ) +
  ...
  plot_LMA(FISod_VEG_CHEM_L2data)
...
# Maak de plots
plot_C <- plot_conc(FISod_VEG_CHEM_L2data, "VEG_CHEM_CONC_C", "C concentration ± SD (gC/kg)")
plot_N <- plot_conc(FISod_VEG_CHEM_L2data, "VEG_CHEM_CONC_N", "N concentration ± SD (gN/kg)")


combined_plot <- (plot_C + plot_spacer() + plot_N) +
  plot_layout(ncol = 3, widths = c(1, 0.05, 1), guides = "collect") +
  plot_annotation(
    title = "Leaf nutrient analysis for C and N at FI-Sod",
    theme = theme(
      plot.title = element_text(face = "bold", size = 14, hjust = 0.5),
...
<p style="font-size: 12px;">
ETC did not yet receive phenocam images for FI-Sod.
